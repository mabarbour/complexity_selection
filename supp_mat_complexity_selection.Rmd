...
gall_selection.df$c.Gall_Height_mm <- with(gall_selection.df, Gall_Height_mm - mean(Gall_Height_mm))

size_GxE_lms <- lm(gall_survival ~ (c.Gall_Height_mm+Genotype+Treatment.focus)^2, data = gall_selection.df, contrast = list(Genotype = "contr.sum"))
summary(size_GxE_lms)
anova(size_GxE_lms)
...
density.grad_models <- plant_agg %>%
  group_by(Treatment.focus, Genotype) %>%
  do(model = lm(gall_survival/mean(gall_survival) ~ I(Density_per_100_shoots - mean(Density_per_100_shoots)), data = .)) %>%
  tidy(model) %>%
  ungroup()
density.grad_models
