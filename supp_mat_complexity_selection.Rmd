...
bootstrap_fitness <- function(logistic_model, newdata, bootstraps, intervals=c(0.025,0.975)){
  ...
  get_intervals <- apply(get_bootstraps$t, 2, function(x) x[order(x)][c(round(bootstraps*intervals[1],0),round(bootstraps*intervals[2],0))])
    
  return(data.frame(newdata, 
                    average = get_absolute_fitness(logistic_model), 
                    lower = get_intervals[1, ],
                    upper = get_intervals[2, ],
                    t(get_bootstraps$t)))
}
```
...
```{r Clutch Size Fitness Landscapes}
...
RF_control_clutch_size <- bootstrap_fitness(
  ...
  newdata = expand.grid(
     sc.gall_size = 0,
     sc.clutch_size = sort(distinct(control_df, Plant_Position, Gall_Number, sc.clutch_size)$sc.clutch_size), # note that these aren't unique
     sc.female_preference = 0),
  bootstraps=100)
...
RF_treatment_clutch_size <- bootstrap_fitness(
  ...
  newdata = expand.grid(
     sc.gall_size = 0,
     sc.clutch_size = sort(distinct(treatment_df, Plant_Position, Gall_Number, sc.clutch_size)$sc.clutch_size), # note that these aren't unique
     sc.female_preference = 0),
  bootstraps=100)
   
## Spaghetti Plot: Relative Fitness ----   
bind_rows(mutate(RF_treatment_clutch_size, Food_Web = "Simple"), 
          mutate(RF_control_clutch_size, Food_Web = "Complex")) %>%
  select(-sc.gall_size, -sc.female_preference) %>%
  gather(ID, fitness, -sc.clutch_size, -Food_Web) %>%
  group_by(ID, Food_Web) %>%
  mutate(relative_fitness = fitness/mean(fitness)) %>%
  ungroup() %>%
  mutate(ID_group = ifelse(ID == "average", "average", "replicate")) %>% 
...
bind_rows(mutate(RF_treatment_clutch_size, Food_Web = "Simple"), 
  ...
  gather(ID, fitness, -sc.clutch_size, -Food_Web) %>%
  mutate(ID_group = ifelse(ID == "average", "average", "replicate")) %>% 
  ggplot(., aes(x=sc.clutch_size, y=fitness, group=interaction(ID,Food_Web), color=Food_Web, alpha=ID_group, size=ID_group)) +
  geom_line() +
  scale_alpha_manual(values=c(1,0.1)) +
  scale_size_manual(values=c(1.5,0.25))
...
```{r Female Preference Fitness Landscapes}
...
RF_control_female_preference <- bootstrap_fitness(
  ...
  newdata = expand.grid(
     sc.gall_size = 0,
     sc.clutch_size = 0, 
     sc.female_preference = sort(distinct(control_df, Plant_Position, sc.female_preference)$sc.female_preference)), 
  bootstraps=100)
...
RF_treatment_female_preference <- bootstrap_fitness(
  ...
  newdata = expand.grid(
     sc.gall_size = 0,
     sc.clutch_size = 0,
     sc.female_preference = sort(distinct(treatment_df, Plant_Position, sc.female_preference)$sc.female_preference)), 
  bootstraps=100)
   
## Spaghetti Plot: Relative Fitness ----   
bind_rows(mutate(RF_treatment_female_preference, Food_Web = "Simple"), 
          mutate(RF_control_female_preference, Food_Web = "Complex")) %>%
  select(-sc.gall_size, -sc.clutch_size) %>%
  gather(ID, fitness, -sc.female_preference, -Food_Web) %>%
  group_by(ID, Food_Web) %>%
  mutate(relative_fitness = fitness/mean(fitness)) %>%
  ungroup() %>%
  mutate(ID_group = ifelse(ID == "average", "average", "replicate")) %>% 
...
bind_rows(mutate(RF_treatment_female_preference, Food_Web = "Simple"), 
  ...
  gather(ID, fitness, -sc.female_preference, -Food_Web) %>%
  mutate(ID_group = ifelse(ID == "average", "average", "replicate")) %>% 
  ggplot(., aes(x=sc.female_preference, y=fitness, group=interaction(ID,Food_Web), color=Food_Web, alpha=ID_group, size=ID_group)) +
  geom_line() +
  scale_alpha_manual(values=c(1,0.1)) +
  scale_size_manual(values=c(1.5,0.25))
