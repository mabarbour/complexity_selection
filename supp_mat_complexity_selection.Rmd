...
gam_red.control_density <- gam(gall_survival_mean ~
                                ...
                                s(c.gall_individuals_mean, k=3) +
                                s(c.Density_per_100_shoots, k=3),   # do I adjust k to be consistent with mixed-effects model?
                             data = control_df_plant.level,
                             family = binomial, #(link = "logit"), 
                             weights = gall_sample_size) # weighting appears to alter the results...
summary(gam_red.control_density)
plot(gam_red.control_density, pages=1, residuals=TRUE)
gam.check(gam_red.control_density)
