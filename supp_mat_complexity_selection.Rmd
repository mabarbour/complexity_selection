...
# following method of Janzen and Stern 
...
glmer_control <- glmer(gall_survival ~ sc.gall_size + sc.clutch_size + sc.female_preference +
                           (1|Genotype/Plant_Position/Gall_Number),
                         family=binomial(link=logit),
                         data = control_df, glmerControl(optimizer="bobyqa"))
...
# Use parametric bootstrapping to get confidence intervals for selection gradients
boot_Beta_control <- bootMer(glmer_control, FUN = Beta_avg_grad, nsim=100, parallel="multicore", ncpus=32)

data.frame(Beta_avg_grad = round(Beta_avg_grad(glmer_control),2),
           lowerCI_2.5 = round(apply(boot_Beta_control$t, 2, FUN = function(bootstraps) quantile(bootstraps, 0.025)),2),
           upperCI_97.5 = round(apply(boot_Beta_control$t, 2, FUN = function(bootstraps) quantile(bootstraps, 0.975)),2)) %>%
  knitr::kable(.)
```
...
# Use parametric bootstrapping to get confidence intervals for selection gradients
boot_Beta_treatment <- bootMer(glmer_treatment, FUN = Beta_avg_grad, nsim=10)

data.frame(Beta_avg_grad = round(Beta_avg_grad(glmer_treatment),2),
           lowerCI_2.5 = round(apply(boot_Beta_treatment$t, 2, FUN = function(bootstraps) quantile(bootstraps, 0.025)),2),
           upperCI_97.5 = round(apply(boot_Beta_treatment$t, 2, FUN = function(bootstraps) quantile(bootstraps, 0.975)),2)) %>%
  knitr::kable(.)
```

```{r}
## Custom function to calculate relative fitness intervals for my models
relative_fitness_intervals <- function(logistic_model, newdata, bootstraps){
  model_matrix <- model.matrix(~ sc.gall_size + sc.clutch_size + sc.female_preference, newdata)
  model_null <- model.matrix(~1, newdata)
  
  get_relative_fitness <- function(.) {
  #mean_fitness <- mean(inverse_logit(model_null %*% fixef(.))) #fixef(update(., .~. -sc.gall_size -sc.clutch_size -sc.female_preference)))
  absolute_fitness <- inverse_logit(model_matrix %*% fixef(.))
  absolute_fitness #/ mean_fitness # mean(absolute_fitness) # 
  }
  
  get_bootstraps <-bootMer(logistic_model, FUN = get_relative_fitness, nsim=bootstraps, parallel="multicore", ncpus=32)
  get_intervals <- apply(get_bootstraps$t, 2, function(x) x[order(x)][c(round(bootstraps*0.025,0),round(bootstraps*0.975,0))])
  
  mean_fitness <- mean(get_relative_fitness(logistic_model))

  return(data.frame(relative_fitness = get_relative_fitness(logistic_model)/mean_fitness,
                    lower_2.5 = get_intervals[1, ]/mean_fitness,
                    upper_97.5 = get_intervals[2, ]/mean_fitness,
                    newdata))
}
```
...
RF_treatment_gall_size <- relative_fitness_intervals(
  ...
  newdata = expand.grid(
     sc.gall_size = seq(min(treatment_df$sc.gall_size),
                        max(treatment_df$sc.gall_size),
                        length.out = 100),
     sc.clutch_size = 0,
...
RF_control_gall_size <- relative_fitness_intervals(
  ...
  newdata = expand.grid(
     sc.gall_size = seq(min(control_df$sc.gall_size),
                        max(control_df$sc.gall_size),
                        length.out = 100),
     sc.clutch_size = 0,
     sc.female_preference = 0),
  bootstraps=200)
...
bind_rows(mutate(RF_treatment_gall_size, Treatment = "Simple"),
  ...
  ggplot(., aes(x=sc.gall_size, y=relative_fitness, color=Treatment, fill=Treatment)) +
...
RF_treatment_clutch_size <- relative_fitness_intervals(
  ...
  newdata = expand.grid(
     sc.gall_size = 0,
     sc.clutch_size = seq(min(treatment_df$sc.clutch_size),
                          max(treatment_df$sc.clutch_size),
                          length.out = 100),
     sc.female_preference = 0),
  bootstraps=200)
...
RF_control_clutch_size <- relative_fitness_intervals(
  ...
  newdata = expand.grid(
     sc.gall_size = 0,
     sc.clutch_size = seq(min(control_df$sc.clutch_size),
                        max(control_df$sc.clutch_size),
                        length.out = 100),
     sc.female_preference = 0),
  bootstraps=200)
...
bind_rows(mutate(RF_treatment_clutch_size, Treatment = "Simple"),
  ...
  ggplot(., aes(x=sc.clutch_size, y=relative_fitness, color=Treatment, fill=Treatment)) +
  geom_hline(yintercept=1, linetype="dotted") +
  geom_line() +
  geom_ribbon(aes(ymin=lower_2.5, ymax=upper_97.5), alpha=0.5)
```
...
RF_treatment_female_preference <- relative_fitness_intervals(
  ...
  newdata = expand.grid(
     sc.gall_size = 0,
     sc.clutch_size = 0,
     sc.female_preference = seq(min(treatment_df$sc.female_preference),
                                max(treatment_df$sc.female_preference),
                                length.out = 100)),
  bootstraps=200)
...
RF_control_female_preference <- relative_fitness_intervals(
  ...
  newdata = expand.grid(
     sc.gall_size = 0,
     sc.clutch_size = 0,
     sc.female_preference = seq(min(control_df$sc.female_preference),
                                max(control_df$sc.female_preference),
                                length.out = 100)),
  bootstraps=200)
...
bind_rows(mutate(RF_treatment_female_preference, Treatment = "Simple"),
  ...
  ggplot(., aes(x=sc.female_preference, y=relative_fitness, color=Treatment, fill=Treatment)) +
  geom_hline(yintercept=1, linetype="dotted") +
  geom_line() +
  geom_ribbon(aes(ymin=lower_2.5, ymax=upper_97.5), alpha=0.5) +
```
...
```{r}
distribution_female_preference <- distinct(.data=gall_selection.df, Plant_Position, Density_per_100_shoots)$Density_per_100_shoots 
HPDinterval_female_preference <- coda::HPDinterval(coda::as.mcmc(scale(distribution_female_preference)), prob=0.95); HPDinterval_female_preference

bind_rows(mutate(visreg(glmer_treatment, xvar="sc.female_preference", scale="response", plot=F)$fit, 
                   Treatment="Simple", `Relative fitness` = visregFit/Wbar_treatment), 
          mutate(visreg(glmer_control, xvar="sc.female_preference", scale="response", plot=F)$fit, 
                   Treatment="Complex", `Relative fitness` = visregFit/Wbar_control)) %>%
  ggplot(., aes(x=sc.female_preference, y=`Relative fitness`, color=Treatment)) +
  geom_line() +
  scale_x_continuous(limits = c(HPDinterval_female_preference[1], HPDinterval_female_preference[2]))
```

```{r}
distribution_clutch_size <- distinct(.data=gall_selection.df, Plant_Position, Gall_Number, gall_individuals)$gall_individuals
HPDinterval_clutch_size <- coda::HPDinterval(coda::as.mcmc(scale(distribution_clutch_size)), prob=0.95); HPDinterval_clutch_size

bind_rows(mutate(visreg(glmer_treatment, xvar="sc.clutch_size", scale="response", plot=F)$fit, 
                   Treatment="Simple", `Relative fitness` = visregFit/Wbar_treatment), 
          mutate(visreg(glmer_control, xvar="sc.clutch_size", scale="response", plot=F)$fit, 
                   Treatment="Complex", `Relative fitness` = visregFit/Wbar_control)) %>%
  ggplot(., aes(x=sc.clutch_size, y=`Relative fitness`, color=Treatment)) +
  geom_line() +
  scale_x_continuous(limits = c(HPDinterval_clutch_size[1], HPDinterval_clutch_size[2]))
```

```{r}
distribution_gall_size <- distinct(.data=gall_selection.df, Plant_Position, Gall_Number, Gall_Letter, Gall_Height_mm)$Gall_Height_mm
HPDinterval_gall_size <- coda::HPDinterval(coda::as.mcmc(scale(distribution_gall_size)), prob=0.95); HPDinterval_gall_size

predict_gall_size_df <- data.frame(sc.gall_size = seq(HPDinterval_gall_size[1], HPDinterval_gall_size[2], length.out=100),
                                   sc.clutch_size = 0,
                                   sc.female_preference = 0)
model_matrix_gall_size <- model.matrix(~sc.gall_size + sc.clutch_size + sc.female_preference, predict_gall_size_df)

# ref: https://www.r-bloggers.com/confidence-intervals-for-prediction-in-glmms/
boot_treatment_gall_size <-bootMer(glmer_treatment, FUN = function(.) inverse_logit(model_matrix_gall_size %*% fixef(.)), nsim=200)
se_treatment_gall_size <- apply(boot_treatment_gall_size$t, 2, function(x) x[order(x)])
boot_lower <- se_treatment_gall_size[1, ]
boot_upper <- se_treatment_gall_size[2, ]


bind_rows(mutate(visreg(glmer_treatment, xvar="sc.gall_size", scale="response", plot=F)$fit, 
                   Treatment="Simple", `Relative fitness` = visregFit/Wbar_treatment), 
          mutate(visreg(glmer_control, xvar="sc.gall_size", scale="response", plot=F)$fit, 
                   Treatment="Complex", `Relative fitness` = visregFit/Wbar_control)) %>%
  ggplot(., aes(x=sc.gall_size, y=`Relative fitness`, color=Treatment)) +
  geom_line() +
  geom_ribbon(aes(ymin=boot_lower, ymax=boot_upper)) +
  scale_x_continuous(limits = c(HPDinterval_gall_size[1], HPDinterval_gall_size[2]))
```
