...
boot_Beta_control <- bootMer(glmer_control, FUN = Beta_avg_grad, nsim=200, parallel="multicore", ncpus=32)
...
# Use parametric bootstrapping to get confidence intervals for selection gradients
boot_Beta_treatment <- bootMer(glmer_treatment, FUN = Beta_avg_grad, nsim=200, parallel="multicore", ncpus=32)
...
relative_fitness_intervals <- function(logistic_model, newdata, bootstraps){
  ...
  get_absolute_fitness <- function(.) {
  absolute_fitness <- inverse_logit(model_matrix %*% fixef(.))
  absolute_fitness 
  }
  
  get_bootstraps <-bootMer(logistic_model, FUN = get_absolute_fitness, nsim=bootstraps, parallel="multicore", ncpus=32)
  get_intervals <- apply(get_bootstraps$t, 2, function(x) x[order(x)][c(round(bootstraps*0.025,0),round(bootstraps*0.975,0))])
  
  mean_fitness <- mean(get_absolute_fitness(logistic_model))

  return(data.frame(relative_fitness = get_absolute_fitness(logistic_model)/mean_fitness,
...
bind_rows(mutate(visreg(glmer_treatment, xvar="sc.female_preference", scale="response", plot=F)$fit, 
                   Treatment="Simple", `Relative fitness` = visregFit/mean(visregFit)), 
          mutate(visreg(glmer_control, xvar="sc.female_preference", scale="response", plot=F)$fit, 
                   Treatment="Complex", `Relative fitness` = visregFit/mean(visregFit))) %>%
...
```{r}
gam.plot(gamm_red.control$gam, select="Gall_Height_mm")
```
