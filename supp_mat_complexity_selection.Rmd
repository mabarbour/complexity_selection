---
title: Supplementary Material for Food-web complexity alters the fitness landscape
...
ADD A SECTION WITH AIC SHOWING THAT ITS BETTER TO USE A MODEL WITH TRANSFORMED VARIABLES, AND IT HELPS MEET THE ASSUMPTIONS OF THE FITNESS LANDSCAPE ANALYSIS.
...
```{r}
length(unique(gall_selection.df$Genotype)) # 8 plant genotypes
length(unique(gall_selection.df$Plant_Position)) # 111 plants
length(unique(gall_selection.df$Gall_Number)) # 614 galls
length(unique(gall_selection.df$Gall_ID)) # 1302 larva
```


We used Aikaike Information Criteria (AIC)

```{r AIC Model Selection, echo=TRUE, cache=TRUE}
...
```{r Check Gradient Calculations, cache=TRUE}
...
```{r Compare Selection Gradients across Treatments - GLMM, cache=TRUE}
...
```{r Compare Selection Gradients across Treatments - gsg GAMM, cache=TRUE}
...
```{r Univariate Fitness Landscapes, cache=TRUE}
## Gall size ----
...
```{r Multivariate Landscapes, cache=TRUE}
...
```{r GLMM Univariate Landscapes, eval=FALSE, include=FALSE}
...
```{r GLMM Multivariate Landscapes, eval=FALSE, include=FALSE}
...
```{r AIC Model Selection v0.1, eval=FALSE, include=FALSE}
...
```{r SELECTION GRADIENTS, echo=TRUE, warning=FALSE, eval=FALSE, include=FALSE}
...
```{r Selection Gradients - Bootstrapped Intervals, eval=FALSE, include=FALSE}
...
```{r Compare bootstrapped replicates across treatments, eval=FALSE, include=FALSE}
bootstrap_df %>%
  filter(Phenotype == "Gall size", Selection_Type == "Beta") %>%
  mutate(ID = rep(1:1000,2)) %>%
...
```{r Fitness Landscapes, eval=FALSE, include=FALSE}
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEzMzk3NTQwOTNdfQ==
-->