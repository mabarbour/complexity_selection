...
The loss of biodiversity is rewiring the web of life; however, we are currently in a poor position to predict how this will affect the ability of populations to evolve and adapt to future environments. To gain insight to these effects, we conducted a field experiment that either maintained a natural community of predators or only allowed one of the predators to impose selection on a common prey. We found that the loss of predators increased the number of prey traits under selection. This suggests that simpler communities may constrain the trajectory of evolution toward a specific combination of traits. Moreover, we found that that the loss of predators could make it more difficult for prey to adapt to future environments. Taken together, our results suggest that the loss of biodiversity may rewire interactions in such a way that constrains the adaptive potential of remaining populations.  
...
**Genetic constraints**: The indirect effects of selection on the G-matrix ($\Delta\text{G}=\text{G}(\gamma - \beta \beta^\text{T})\text{G}$) is governed by the curvature of the adaptive landscape ($\text{C}=\gamma - \beta \beta^\text{T}$), which in our study corresponds to:
...
```{r EggPtoid_Selection, echo=FALSE, fig.pos="H", fig.cap="\\label{fig:EggPtoid_Selection}Selection imposed by larval parasitoids on egg parasitoids (*Platygaster* sp.). The bold line represents the average difference in the probability of observing the egg parasitoid (original - removal of larval parastioids) as a function of gall midge oviposition preference. Thin lines represent bootstrapped replicates to show the uncertainty in selection. For clarity, we only display 100 bootstraps even though inferences are based on 1,000 replicates. The decrease in the probability of observing egg parasitoids at high gall-midge densities indicate that larval parasitoids impose nonlinear selection on egg parasitoids."}
