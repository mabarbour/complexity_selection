...
Knowledge of the curvature matrix alone gives an incomplete picture of its indirect effect on genetic constraints. This is because genetic constraints are ultimately determined by the structure of the G-matrix, and thus may depend on its structure before selection. Although we do not know the underlying G-matrix for the traits we measured in this experiment, we can still gain insight to how our food-web treatment would alter genetic constraints. Specifically, we first calculated how our best estimate of the curvature matrix (mean values) in each treatment changed the structure of `r n_Gmats` random G-matrices. If we assume that recombination/mutation would have the same effect on gall midge populations in the original and extinction food webs, then we can estimate how the curvature matrix in the previous generation alters the G-matrix for the next generation.

The G-matrix itself is a complex structure, but recent theory has shown how its structure affects the evolvability of a population [@Hansen2008]. <!--We used evolvability as our measure of genetic constraint, as it has a solid foundation in theory of quantitative genetics and adaptive landscapes [@Hansen2008]. -->Evolvability measures the ability for a trait to evolve toward a given direction of selection, ignoring responses in every other direction [@Hansen2008]. In the absence of knowledge about the direction of selection that a population will actually experience in the next generation, we can assess its evolutionary potential by computing its average evolvability over random selection gradients (here, we used `r n_random_gradients`). By computing the average evolvability for each of `r n_Gmats` random G-matrices, we can then assess how changes in the curvature matrix alter the evolutionary potential of a population. We report the distribution of these effect sizes, rather than conduct a statistical test, because the number of replicates in a simulation can be arbitrarily high, thus making any effect size statistically significant [@White2014].  
...
`r round(filter(biased_foodweb_confint, term=="FoodwebOriginal:sc.Diam")$conf.high,2)`]; extinction $\alpha_{\text{Diam}}$=
...
`r -1*round(filter(tidy_foodweb_alphas, type=="Diff", term=="sc.Diam")$conf.high,2)`]), but was simply a consequence of the (expected) lower survival in the original food web <!--(original $\bar W$=
...
## Consumer extinctions tend to increase genetic constraints
...
`r round(filter(tidy_egglarval_grads, type=="Original", term=="I(sc.sqrt.Pref^2)")$conf.high,2)`]), but was also magnified by the lower average survival in complex food webs.
...
Of the different components of the curvature matrix, we found that only the curvature of oviposition preference differed between food-web treatments. Specifically, selection in the extinction food web acted to decrease the genetic variation in preference relative to the original food web.
...
Interestingly, when we translate the effect of the curvature matrix onto the G-matrix and evolvability of the next generation, we see that the extinction food web had a general tendency to decrease evolvability (fig. \ref{fig:Evolvability}). Specifically, the extinction food web decreased the average evolvability of `r round(sum(random_evolvabilities$Diff < 0) / length(random_evolvabilities$Diff) * 100,0)`% of the random G-matrices in our simulation. If anything, we expect that this underestimates the true effect of our extinction treatment. For example, if we assume egg parasitoids would eventually impose similar impact on mean fitness once they are released from intraguild predation (i.e. extinction $\bar W$=original $\bar W$), then the extinction food web decreases the average evolvability in `r round(sum(long_random_evolvabilities$Diff < 0) / length(long_random_evolvabilities$Diff) * 100,0)`% of the G-matrix scenarios.
...
```{r Evolvability, echo=FALSE, fig.pos="H", fig.cap="\\label{fig:Evolvability}Change in average evolvability for 10,000 random G-matrices using our best (mean) estimate of the curvature matrix for each food-web treatment. We found that the curvature of the extinction food web decreased evolvability in 71% of the G-matrices, suggesting that consumer extinctions tended to decrease evolutionary potential in our study."}
