...
The curvature ($\gamma - \beta \beta^\text{T}$) of the adaptive landscape describes how natural selection changes genetic variation ($\Delta \text{G}$) within a generation. 

In the simple food web, we found that the adaptive landscape acted to decrease additive genetic variance in all three traits (diagonals of matrix). 

$$\text{Curvature}_{simple} = \begin{pmatrix} -0.05 & 0.02 & 0.03 \\  0.02 & -0.01 & -0.01 \\  0.03 & -0.01 & -0.02 \end{pmatrix}$$

In contrast, we found the adaptive landscape in the complex food web only decreased the additive genetic variance in one of the three traits (chamber diameter).

\text{Curvature}_{complex} = \begin{pmatrix} -0.13 & 0 & 0.9 \\ 0 & 0 & 0 \\ 0.09 & 0 & 0.33 \end{pmatrix}

In the complex food web, we found that the adaptive landscape  We found that the adaptive landscape complex food web, 

We found that there were fewer selective constraints influence *Iteomyia* in the complex vs. the simple food-web treatment. Specifically, the curvature of the complex food-web was described as:



whereas the curvature of the simple food-web is summarized by the following matrix:
...
\subsection*{Phenotypic evolution is less constrained in the complex food web}

% of the adaptive landscape describe its shape and therefore the direction and magnitude of phenotypic ($\Delta \text{z}$) and genetic change ($\Delta \text{G}$).

Three key patterns emerged from our simulations of phenotypic evolution. The first was that phenotypic evolution was faster in complex vs. simple food webs. Specifically, it took \textit{Iteomyia} twice as long to hit the boundary of the adaptive landscape in the simple vs. complex food web. This pattern was driven by the fact that the absolute strength of selection was larger in complex vs. simple food webs, as we no longer observed this strong difference if we standardized all selection gradients by average larval survival measured in the complex food web (Appendix).

The second key pattern we observed was that phenotypic evolution was less constrained in complex compared to simple food webs.

Finally, we found that the adaptive landscape in the complex food web enhanced the evolvability of \textit{Iteomyia} compared to the simple food web.

Food-web complexity fundamentally altered the shape of the adaptive landscape. For example, 

Across both food-web treatments, there was no clear evidence of correlational selection between (Table 1).

We observed disruptive selection on female preference in the complex, but no evidence of any nonlinear selection the simple food web. This disruptive selection was likely driven by a different functional response of larval parasitoids to female preference. For example, when we subset our data to focus on differences between larval and egg parasitoids, we found that there was a trend for larval parasitoids to impose disruptive selection (\gamma_{pref,pref}=0.22 [-0.06,0.49]). 

In both complex and simple food webs, chamber diameter was under strong directional selection with larger galls resulting in higher larval survival (fig.~\ref{Fig:FL_1D}; table~\ref{Table:Gradients}).

In general, if there was evidence of selection, then the magnitude of selection was always greater in complex vs. simple food webs. These steeper selection gradients were simply due to the fact that average larval survival was lower in complex food webs.
