...
beta_control <- glmer(gall_survival ~ sc.gall_size + sc.clutch_size + sc.female_preference +
...
# Quadratic selection ----
quad_control <- update(beta_control, .~. + I(sc.gall_size^2) + I(sc.clutch_size^2) + I(sc.female_preference^2))

summary(quad_control)
overdisp_fun(quad_control)

# Correlational selection ----
corr_control <- update(quad_control, .~. + sc.gall_size:sc.clutch_size + sc.gall_size:sc.female_preference + sc.clutch_size:sc.female_preference)

summary(corr_control)
overdisp_fun(corr_control)

# Mean fitness ----
Wbar_control <- inverse_logit(fixef(update(beta_control, .~. -sc.gall_size -sc.clutch_size -sc.female_preference)))
...
# Directional selection ----
beta_treatment <- glmer(gall_survival ~ sc.gall_size + sc.clutch_size + sc.female_preference +
                           (1|Genotype/Plant_Position/Gall_Number),
                         data = treatment_df,
                         family = binomial(link = logit), control=glmerControl(optimizer = "bobyqa"))
...
# Quadratic selection ----
quad_treatment <- update(beta_treatment, .~. + I(sc.gall_size^2) + I(sc.clutch_size^2) + I(sc.female_preference^2))

summary(quad_treatment)
overdisp_fun(quad_treatment)

# Correlational selection ----
corr_treatment <- update(quad_treatment, .~. + sc.gall_size:sc.clutch_size + sc.gall_size:sc.female_preference + sc.clutch_size:sc.female_preference)

summary(corr_treatment)
overdisp_fun(corr_treatment)

# Mean fitness ----
Wbar_treatment <- inverse_logit(fixef(update(beta_treatment, .~. -sc.gall_size -sc.clutch_size -sc.female_preference)))
...
```{r Table of Selection Gradients, echo=FALSE, message=FALSE, warning=FALSE}
...
bind_rows(summary_beta_control, summary_beta_treatment)
bind_rows(summary_quad_control, summary_quad_treatment)
bind_rows(summary_corr_control, summary_corr_treatment)
```
...
```{r Figure 1, echo=FALSE, fig.cap="Illustrations of complex (A) and simple (B) food webs associated with the insect herbivore, *Iteomyia salicisverruca*."}
...
RF_control_gall_size <- bootstrap_fitness(
  ...
  fixed_effects = "~ sc.gall_size + sc.clutch_size + sc.female_preference",
  newdata = expand.grid(
...
RF_uni_size <- bind_rows(mutate(RF_treatment_gall_size$relative_fitness, Food_Web = "Simple"), 
  ...
  scale_y_continuous(breaks = RF_gradient_scale, limits = c(min(RF_gradient_scale), max(RF_gradient_scale)), labels = RF_gradient_scale) +
...
```{r Figure 2 Univariate Landscapes, echo=FALSE, fig.cap="Selection gradients acting on gall traits in complex vs. simple food webs. Each panel corresponds to a different gall trait: gall diameter (A); clutch size (B); and female preference (C). Solid lines represent the estimated gradients in complex (orange) and simple (blue) food webs. Transparent lines represent bootstrapped replicates (n=100) to show the uncertainty in estimated gradients."}
...
```{r AIC Model Comparison, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}

# Control ----
AIC(beta_control, quad_control, corr_control)

glm.beta_control <- glm(gall_survival ~ sc.gall_size + sc.clutch_size + sc.female_preference, data=control_df, family=binomial(link=logit))

beta_control.REonlyGallID <- glmer(gall_survival ~ sc.gall_size + sc.clutch_size + sc.female_preference +
                                   (1|Gall_Number),
                                 data = control_df,
                                 family = binomial(link = logit), control=glmerControl(optimizer = "bobyqa"))
...
glm.beta_treatment <- glm(gall_survival ~ sc.gall_size + sc.clutch_size + sc.female_preference, data=treatment_df, family=binomial(link=logit))

beta_treatment.REonlyGallID <- glmer(gall_survival ~ sc.gall_size + sc.clutch_size + sc.female_preference +
...
```{r Univariate Absolute Fitness Landscapes, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r Figure S1 Univariate Absolute Fitness Landscapes, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r Figure S2 Multivariate Absolute Fitness Landscapes, echo=FALSE, message=FALSE, warning=FALSE}
# Get legend
...
gam.gradients(gamm_control$gam, phenotype=c("sc.gall_size","sc.clutch_size"), covariates = c("sc.female_preference"), se.method="none")
gam.gradients(gamm_control$gam, phenotype=c("sc.gall_size","sc.female_preference"), covariates = c("sc.clutch_size"), se.method="none")
gam.gradients(gamm_control$gam, phenotype=c("sc.clutch_size","sc.female_preference"), covariates = c("sc.gall_size"), se.method="none")
