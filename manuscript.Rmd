...
\noindent 1. University of British Columbia, Department of Zoology, Vancouver, BC V6T 1Z4, Canada; <!-- consider removing -->
...
*Keywords*: fitness landscape, host-parasitoid, natural selection, G-matrix, community context, ecological networks, evolutionary ecology, eco-evolutionary dynamics.
...
**Evolutionary constraints**: We quantified the effects of food-web complexity on evolutionary constraints by inspecting the slope and curvature of the adaptive landscape. <!--Adaptive landscape**: We took advantage of existing theory that translates estimates of selection gradients to the adaptive landscape [@Phillips1989; @Arnold2003]. The shape of the adaptive landscape can be described by both its slope and curvature, each of which describe different aspects of constraints on phenotypic evolution. Specifically, -->The number of selective constraints is determined by the slope of the adaptive landscape<!-- corresponds to the column vector of directional selection gradients-->, which in our study corresponds to:
...
<!--This slope describes the selective constraints acting on the trajectory of phenotypic evolution, which is governed by the 'Lande equation', $\Delta \bar z=\text{G}\beta$ [@Lande1979]. Quantitative predictions of evolutionary trajectories require knowledge of a population's **G**-matrix; however, We quantified the degree to which phenotypic evolution was constrained by selection <!--can gain qualitative insight to evolutionary constraints by counting the number of phenotypic traits under directional selection [selective constraints, @Arnold1992].-->
By comparing the number of traits that determine the slope (i.e., nonzero directional selection gradients) in simpler vs. more complex food webs, we can infer the effect of food-web complexity on selective constraints. 

The indirect effects of selection on genetic constraints is governed by the curvature of the adaptive landscape ($\text{C}=\gamma - \beta \beta^\text{T}$), which in our study corresponds to:
...
Remember that we can infer the indirect effects of selection on genetic constraints by counting the number of negative signs along the diagonal (decrease in additive genetic variance) and the number of nonzero terms along the off-diagonal (trait integration) of the curvature matrix. The structure of these matrices indicates that there are more selective constraints imposed on the gall midge in the simpler (6 of 6) vs. more complex (1 of 6) food web. Simpler food webs acted to decrease genetic variance for all three phenotypic traits (negative diagonal terms), whereas only one trait (chamber diameter) experienced a decrease in additive genetic variance in the more complex food web. For genetic covariances, the simpler food web favored integration among all three phenotypic traits (nonzero off-diagonal terms), and thus constraints along all three axes of covariance (fig. \ref{fig:MV_Landscape}). In contrast, there was no strong evidence of trait integration in the more complex food web (fig. \ref{fig:MV_Landscape}).


```{r MV_Landscape, echo=FALSE, fig.pos="H", fig.cap="\\label{fig:MV_Landscape}Two dimensional view of adaptive landscapes of gall midge phenotypes in complex vs. simple food webs. Each panel corresponds to a different combination of phenotypic traits: clutch size and chamber diameter (A); clutch size and oviposition preference (B); oviposition preference and chamber diameter (C). Arrows represent mean estimates of directional selection gradients, while contours represent predicted larval survival of the mean phenotype (i.e., all traits = 0) in each food-web treatment. Notice that arrows point more toward a corner of the adaptive landscape for each combination of traits in the simple vs. complex food-web treatment. This indicates that trait integration (covariance) is more strongly favored in simpler vs. more complex food webs. Note that mean larval survival is plotted on a natural log scale to reflect the mathematical definition of the adaptive landscape."}
knitr::include_graphics("analyses/MV_landscapes.pdf")
```

## Selection on the extended phenotype of egg parasitoids

<!-- CONSIDER AVOIDING EXTENDED PHENOTYPE AND FOCUS ON INDIRECT EFFECTS OR INDIRECT SELECTION -->
...
```{r EggPtoid_Selection, echo=FALSE, fig.pos="H", fig.cap="\\label{fig:EggPtoid_Selection}Selection imposed by larval parasitoids on the extended phenotype of egg parasitoids (*Platygaster* sp.). The bold line represents the average difference in the probability of observing the egg parasitoid in complex vs. simple food webs as a function of gall midge oviposition preference. Thin lines represent bootstrapped replicates to show the uncertainty in selection. For clarity, we only display 100 bootstraps even though inferences are based on 1,000 replicates. The decrease in the probability of observing egg parasitoids at high gall-midge densities indicate that larval parasitoids impose nonlinear selection on the extended phenotype of egg parasitoids."}
...
<!--Summary-->
<!-- Our key finding was that phenotypic evolution in gall midges was more constrained in simpler food webs. More traits contributed to the slope of the adaptive landscape in the simpler food web, suggesting that the trajectory of phenotypic evolution is more constrained. -->
We found that simpler food webs constrained phenotypic evolution in gall midges in two key ways. First, more traits contributed to the slope of the adaptive landscape in the simpler food web, suggesting greater constraints on the trajectory of phenotypic evolution. Second, the simple food-web treatment indirectly increased the number of genetic constraints, which could act to constrain the gall midge's adaptive potential in the face of novel selection pressures. We also found evidence of indirect selection pressures in more complex food webs, suggesting that the adaptive landscape may be more dynamic in complex food webs. Taken together, our study provides experimental evidence from the field that the simplification of ecological communities may constrain the adaptive potential of remaining populations.

<!--Selective constraints-->
All three phenotypic traits we examined experienced directional selection in the simpler food web, indicating that there is an optimal phenotype that maximizes larval survival (i.e. large chamber diameter, small clutch size, and avoidance of conspecifics). In contrast, we did not observe selective constraints on clutch size and oviposition preference in the more complex food web. This suggests that there are many optimal phenotypes (adaptive peaks), or in this case, a flat adaptive landscape where there are no fitness consequences for phenotypic change in these traits. This also implies that as selective constraints dampen in more complex food webs, then the trajectory of evolution becomes more determined by genetic constraints. This is made clear if we inspect the 'Lande equation', $\Delta z = \text{G} \beta$, where G corresponds to the population's G-matrix. As the components of $\beta$ go to zero, then $\Delta z$ is more influenced by the G-matrix. Interestingly, this aligns with @Schluter1996's hypothesis that phenotypic evolution often follows 'genetic lines of least resistance'. @Schluter1996 found support for this hypothesis from data on natural populations of several vertebrate species, including threespine sticklebacks, a few species of songbirds, and mice from the genus *Peromyscus*. All of these species occupy intermediate trophic levels and are likely embedded in complex food webs, which is consistent with our suggestion that genetic constraints may have a stronger influence in more complex food webs. 

<!--Indirect effects of selective constraints-->
We also found evidence for more genetic constraints in simpler food webs due to indirect effects of selection on the population's G-matrix. The ability of a population to adapt to novel selection pressures (evolvability) is largely governed by the structure of its G-matrix [@Hansen2008]. When selection favors genetic covariance between traits (positive or negative), this results in less autonomy of evolutionary responses to changing environments. Similarly, decreases in genetic variance constrain potential for the trait itself to evolve. Together, this suggests that simpler food webs may decrease the evolutionary potential of populations by indirectly selecting for decreases in genetic variance in multiple traits and favoring trait integration. Current theory often assumes genetic variances and covariances remain constant over time rather than dynamically changing with the community context [@McPeek2017; @Guimaraes2017]. Our empirical results highlight the need to explore the evolutionary consequences of not only direct effects of selection, but indirect effects on genetic constraints that are shaped by the community context.

<!-- Generality of results -->
The generality of our results likely depends on the relative abundance and functional differences between consumers in a community. In the simplest case, when consumers do not differ from each other, then we do not expect changes in food-web complexity to modify selective constraints. Also, many consumers may be at too low of abundances to impose selection on their resources. Rank abundance curves [@Preston1948] and the disproportionate number of weak interactions in diverse communities [@Paine1992] support this notion. When consumers are abundant though, the effect of food-web complexity will depend on whether different species impose conflicting selection pressures or select for distinct traits. For example, parisitoids and birds impose conflicting selection pressures on the size of galls induced by the fly *Eurosta solidaginis* [@Weis1985; @Abrahamson1997; @Start2016]. Recent studies in this system have shown that decreases in the relative abundance of birds, due to either small patch sizes [@Start2016] or proximity to urban areas [@Start2018] causes a shift from neutral to directional selection on gall size. On the other hand, different consumers may impose selection on different traits, favoring trait integration and increasing genetic constraints. Examples of this include strong genetic covariances in plant resistance to different insect herbivores [@Maddox1990; @Wise2007; @Wise2013], although there are also examples where these covariances are weak [@Roche1997; @Barbour2015], or vary from year-to-year [@Johnson2007]. We suggest that gaining predictive insight to the effects of food-web disassembly requires an understanding of the mechanisms governing the assembly of trophic interactions.

<!--Indirect evolutionary effects-->
Our results suggest that the simplification of ecological communities may not only directly affect connected species, but also result in indirect evolutionary effects. In our study, this indirect effect arises from egg parasitoids being released from intraguild predation in simpler food webs. This release occurs more on trees with high larval densities, which could intensify selection on gall midge oviposition preference. A growing number of experiments over the past two decades have demonstrated the presence and potential importance of indirect evolutionary effects that emerge in a community context [@Juenger1998; @Stinchcombe2001; @Lankau2007; @Walsh2008; @Walsh2010; @terHorst2010; @Sahli2011; @Lau2012; @terHorst2015; @Schiestl2018]<!--; Start et al. in press-->. If indirect evolutionary effects are common [@Walsh2013], then predicting evolutionary trajectories resulting from the simplification of food webs will require evolutionary studies to explicitly account for the ecological networks that species are embedded in. 

<!-- Community context shaping adaptation to environmental change --> 
There is a growing number of theoretical studies on adaptation to directional environmental change that incorporate species interactions [e.g. @deMazancourt2008; @Johansson2008; @Norberg2012; @Osmond2017PredatorsHelpPrey]. This work has given insight to the mechanisms by which pairwise interactions modify the adaptive potential of species. Our study hints at a novel mechanism, whereby food-web complexity flattens the adaptive landscape, thus facilitating future adaptation by allowing genetic and phenotypic variation to persist. This mechanism only emerges once we move beyond pairwise interactions to consider selection on multiple traits in a community context. However, a study on competition has highlighted that we may expect the opposite effect of species diversity in competitive communities [@deMazancourt2008]. This negative effect of species diversity on adaptation occurs because there is a greater chance that the optimal phenotype is already occupied by a competitor in a more diverse community. More work is needed to examine how the distribution of different interaction types affects adaptation to environmental change in species-rich communities. 

<!-- Conclusions -->
Our study gives insight to how local extinctions, and concomitant decreases in food-web complexity, increase evolutionary constraints. This hints at a potential insidious effect of local extinctions that compromises the robustness of remaining populations to future environmental change. Our work also highlights some key challenges for predicting phenotypic evolution in the face of global change. First, the simplification of ecological communities may actually reduce the predictability of phenotypic evolution, because knowledge is required of both selective and genetic constraints, rather than potentially just genetic constraints in more complex systems. Second, many theoretical models of eco-evolutionary dynamics focus on phenotypic change in a single trait, yet our results highlight that the number of traits under selection may change with the community context. Importantly, while the net effects of selection in complex communities may be dampened, we still found that different species/guilds imposed different selection pressures. Knowing these hidden selection pressures is critical for prediction, because the trajectory of evolution will depend on the nature of change in the community context. We expect that a continued integration of adaptive landscapes and ecological networks will enhance our ability to predict the evolutionary consequences of changes in ecological communities. 


<!--
Summary of key results

Gall midges experience more selective and constraints in simple food webs.
-->
<!--More phenotypic traits experienced directional selection in the simple food-web treatment, suggesting that the trajectory of phenotypic evolution is more constrained.-->

<!--, because the egg parasitoids that attack larva on high density trees have a higher chance of surviving.-->

<!--In the context of global change, our results imply that the simplification of ecological communities may actually reduce the predictability of phenotypic evolution of remaining populations. This is because knowledge is required of both selective and genetic constraints, rather than potentially just genetic constraints in more complex systems.-->

<!-- what should the focus be? 
Conceptually, I don't think it'll be that interesting to try and review past studies.

I think it would be more interesting to discuss the implications of our results and there generality. In other words, under what conditions do we expect them to hold. When will they break down? 

Highlight that we could gain more predictive power from theoretical models that also examine how selection modifies selection pressures of consumers. i.e. we need models of ecological character displacement that go beyoned pairwise interactions. In some communities, intraguild predation will be common so we also need models that examine how the community context shapes the evolution of trophic levels and the resulting feedbacks on associated communities (cite JP's Frontiers in Ecology and Evolution MS).

Such indirect effects may be quite common when moving from complex to simple communities, and highlights the fact that species evolve in a community context.<!-- and the release of selection pressures may have rippling effects.-->  <!--If indirect evolutionary effects are common [@Walsh2013], then predicting the evolutionary trajectories of remaining populations after local extinctions will not only require knowledge of these indirect effects, but also require knowledge on the order of local extinctions.

It is well known that increases in consumer diversity can driver character divergence in foraging traits due to ecological character displacment [@Schluter2000]. In this case, we would expect the local extinction of a consumer to 
- Ecology example: selection on gall size in Eurosta
- Evolution example: ecological character displacement tends to drive divergence.
-->

<!--
Detailed case studies of natural selection on a single trait in a community context highlight the potential generality of our results. For example, galls induced by the fly *Eurosta solidaginis* experience directional selection for larger sizes by parasitoids (as we observed in our study), but selection for smaller sizes by birds [@Weis1985; @Abrahamson1997; @Start2016]. Therefore, the dynamics of natural selection on gall size depend on the relative abundance of these consumers. A recent study in this system demonstrated that the loss of birds across a gradient of urbanization caused a shift from neutral to directional selection on gall size [@Start2018]. This observational study supports our experimental demonstration that the loss of consumer diversity increases selective constraints. 

In a different system, the direction of selection on swimming speed in *Enallagama* damselflies depends on whether they occur in fish or fishless lakes [@McPeek1996; @McPeek1997]. Lakes with fish select for damselflies , whereas dragonflies become the top predator in fishless lakes and select for adaptations to increase swimming speed.  

fish and dragonflies impose opposite directional selection on swimming speed in *Enallagma* damselflies . This has resulted in 

Lakes with fish likely have a much more diverse community of damselfly predators (multiple fish species and some smaller dragonfly species), 

Our study focused on an experimental manipulation of a focused community of consumers, leaving an open question to how our results generalize to more diverse consumer communities. 

-->
<!--
The generality of our results depends on the extent to which different consumer species are functionally different and their relative abundance. In the simplest case, when consumers do not differ from each other (i.e. neutral communities), then we do not expect changes in food-web complexity to modify selective constraints. Also, many consumer species may be at too low of abundances --><!--This may be the case with many consumers in diverse communities, where their abundances are simply not high enough --><!-- to impose selection on the mean trait values of resource populations. Rank abundance curves [@Preston1948] and the disproportionate number of weak interactions in diverse communities [@Paine1992] support this notion. For abundant species though, the effect of food-web complexity depends on whether different species impose selection on different trait values vs. distinct traits.

Current theory on character displacement suggests that consumers will usually diverge in foraging traits when foraging on a biological resource. On one end, theory should be extended beyond two consumers to examine trait dynamics in more diverse consumer communities. On the other hand, we also need to examine selection in the resources themselves. 
-->
<!--
Actually, different predator species impose selection on different axes of trait variation. 
For example, habitat shifts into lakes where dragonflies are the top predator (instead of fish) promote adaptations for increased swimming speed in multiple lineages of *Enallagma* damselflies [@McPeek1996].
Phenotypic selection study with damselflies [@McPeek1997].
Biochemical evolution study with damselflies [@McPeek1999], still related to swimming speed.
In McPeek's study though, there aren't dragonflies in lakes with fish?

Lakes with fish are the ancestral state, and they move to fishless lakes. Although the fishless lakes may have simpler communities, we see a clear effect of the dominance hierarchy which highlights the importance of abundance. In this case, we have a strong intraguild predator that reduces the density of intraguild prey (dragonfly larvae), thus reducing their selection pressures. This indicates that its' not simply the presence/absence of different predators, but the evenness of these selection pressures.
-->
<!--
Maybe they break down with plant-herbivore interactions? Or maybe these provide a useful example for thinking about when they'll break down or for highlighting indirect effects on genetic constraints?

We suggest that being able to predict how changes in community context shape evolutionary constraints requires a deep understanding of the ecological and evolutionary feedbacks that shape trait variation and community structure.

Here, we focus on how changes in food-web complexity affects evolutionary constraints on a single resource population. However, predicting h

Predicting how evolution occurs in a community context thus requires an explicit examination of the eco-evolutionary feedbacks that propogate through the networks. The textbook example of such a feedback in structuring communities is ecological character displacement, "the process of phenotypic evolution in a species generated or maintained by resource competition with one or more coexisting species" [@Schluter2000]. In scenarios, that promote character divergence in consumers, we would expect those consumers to impose selection on different axes of trait variation (although typically, a resource spectrum is used), resulting in more neutral selection on traits compared to a community with only one consumer. The character displacement framework would provide a useful starting point for theoretical studies that explore feedbacks between evolutiona and ecology in a community context [@McPeek2017]. Importantly though, our results suggest that it would be useful in exploring multiple traits. 
In other words, we need to be able to predict not just how diversity itself affects phenotypic evolution of resources, but also of consumers. The body of research on character displacement provides a useful starting point, as there have been extensive examinations of how community context shapes phenotypic evolution of competing consumers [Abrams work, Taper and Case's work]. Still, much of this work has focused on pairwise competitive interactions, with little exploration of the direct and indirect effects that may occurs in competitive communities [@Levine2017]. 
-->
<!--
rather than how the complexity of interactions itself shapes the adaptive landscape, although a careful review of these studies can give valuable insights. For example, we found that fewer traits were under selection in complex vs. simple communities. In cases whether natural enemies impose selection on different axes of trait variation, we would expect complex communities to neutralize selective pressures. Recent work of pollinator communities [@Sahli2011; @terHorst2015; @Schiestl2018] has similarly shown a tendency for more traits to experience directional selection in simpler vs. more complex pollinator communities. 

@Stinchcombe2001 found that there was selection for increased deer resistance only in the presence of insects rather than their absence.

For example, @Juenger1998 found that flies and caterpillars imposed the strongest reduction in plant fitness in isolation, but these effects were minimized when they were together, highlight a nonadditive interactions that result in neutral selection on flower time when both were present (unclipped treatment)

However, @Lankau2007 only observed selection on chemical concentration of *Brassica* when both herbivores were present, but only when there was a heterospecif neighbor. In contrast, in the presence of conspecifics, then aphids and mollusks imposed contrasting selection pressures, resulting in no net selection. These different responses appear to be due to differences in diet breadth of mollusks and aphids.

Schiestel et al. Evolution 2018: In the generalized treatment, only 1 axis of trait variation had a nonzero selection gradient. In the bumblebee only, there were 2-5 trait axes (depending whether you count marginal significance), in the hoverfly, there was only 1 trait axis. Interestingly, pollinator network complexity appears to dampen the selection gradients imposed by the most efficient pollinator (bumblebee), so I would say the interpretation is consistent. Here there appear to be some non-additive selection pressures at play. [greenhouse study with experimental manipulation of community]

in terHorst 2010 Am. Nat, they also observed that increasing complexity often cancelled out the selection pressures shown by each species in isolation (e.g. cell area and population growth, or cell speed), although here, there appears to be non-additive selection pressures at play. Indeed, the indirect effects often result in little net evolution of multiple traits in multispecies populations that experienced both direct and indirect effects. (so it is tough to say for my experiment what might happen if we remove the parasitoids altogether...)[lab study with protist microcosms]

Walsh's Rivulus work (Walsh and Reznick, 2008 and 2010) as well as Start's work is more of an indirect effect due to a specific selection pressure (predator then resources, and for Start's a herbivore competitor).[start was a field experiment, Walsh's work took advantage of evolved pops in different contexts, also manipulated food availability to tease apart direct vs. indirect eco-evo effects].

There is also a study by Lau 2012 in Oecologia, but it's hard to follow. [field study with experimental manipulation of community]. Actually, the reanalysis by terHorst et al. 2015 suggests that selection for resistance follows the observed pattern (dampened selection in more complex community).

Again for Sahli and Conner (2011 in Evolution), in the presence of all three pollinators, we lacked selection on Exsertion and Dimorphism. This evidence is quite compelling and interesting...
 
Studies like Wise and Rausher show wide spread evidence for diffuse coevolution, but not through community members altering selection pressures, but via genetic covariances in resistance to herbivore species. They argue that diffuse interactions had an overwhelmingly constraining rather than facilitat effect, which may well be the case with a lot of covariances. [fiel study, no experimental manipulation of community]

Lankau and Strauss 2007: (community complexity drives patterns of natural selection on a chemical defense of brassica nigra). 

Mauricio and Rausher 1997 Evolution did a field study where they sprayed the field and looked at selection. Interestingly, they only observed stabilizing selection (i.e. indirect genetic constraints) on glucosinolate concentration in the simply food web, but in general the selection pressures were quite similar...somthing funky with their analyses? Perhaps though this is again the argument of measuring selection in a "disturbed" community...
-->

<!--How does my study improve or differ from these? Perhaps the focus on 
<!-- 
Natural history and comparison to other studies of natural selection in galling insects. 

Important Studies for Comparison: @Abrahamson1997; @Weis1983; @Heath2018; Start, Weis, and Gilbert In Press Am. Nat; @Start2016; @Start2018urbanization; Peter Price's work; Timothy Craig's work; Cheryl Briggs work.

Topics to discuss: Persistent pattern in selection for larger galls; Contrasting effects of egg and larval parasitoids on clutch size; Nonlinear selection at high gall densities is likely reflective of the poorer searching ability of larval vs. egg parasitoids (and thus it saturates more quickly).  
-->

<!--
Our finding that phenotypic evolution was more constrained in simpler food webs corresponds to the natural history of this system. Simpler food webs selected for gall midges with smaller clutch sizes and ovipositing on trees with low densities of conspecifics, which was likely due to the increased apparency of large clutches and trees with high densities to egg parasitoids searching for hosts. This is because herbivore oviposition can induce plant volatiles that attract egg parasitoids [reviewed in @Hilker2006], and the concentration of these volatiles may increase with the number of eggs that herbivores oviposit. In contrast, clutch size and oviposition preference did not experience directional selection in more complex food webs, which appeared to be a result of conflicting selection pressures.  The absence of selection on clutch size 


Interestingly, we still observed selection for larger chamber diameters in the simple food web despite the fact that egg parasitoids attack before the gall is formed. This suggests that selection imposed by egg parasitoids on chamber diameter is likely due to selection with a correlated trait (e.g. ability to encapsulate parasitoid eggs).

This particular gall-parasitoid community has only been studied one other time [@Barbour2016]. But a persistent pattern seen in others work is that parasitoids generally impose selection for larger galls [@Abrahamson1997; @Heath2018; @Start2016; @Start2018urbanization]. In many cases, there may be functional or development constraints acting on gall size that constrains phenotypic evolution. It is curious though that we still observed directional selection for larger chamber diameters in the simpler food web. This is because egg parasitoids attack before the gall is formed. Thus, the apparent selection on chamber diameter is likely due to selection with a correlated trait (e.g. ability to encapsulate endoparasitoid eggs). In terms of clutch size, we likely observe conflicting selection pressures for the following reason. First, egg parasitoids reduce the probability of larval survival in higher clutch sizes, because they cue in more to the local density when foraging. Indeed oviposition itself can induce plant volatiles that influence egg parasitoid foraging behavior [reviewed in @Hilker2006]. In contrast, clutch sizes can increase the probability of larval survival in the face of larval parasitoids by creating more of a refuge from parasitoids that have to oviposit through the gall tissue. Similar patterns in larval parasitoids has been observed for other a related gall midge species [@Weis1982]. Alternatively, in another gall midge, the same reduction in percent parasitism has been shown [@Weis1983]. This previous work did not explore a relationship with the egg parasitoid Platygaster because it was too low in abundance, which is in contrast to our community where this species is the dominant parasitoid. Counterintuitively, this may not be due to a greater refuge, but simple a result of the parasitoids using a fixed-time strategy to search for hosts. Thus, parasitoids actually have a lower success attacking small galls, but a good chance of attacking one host in large galls and then just moving on to the next. In regard to oviposition preference, our results correspond to biological differences between egg and larval parasitoids. For example, egg parasitoids, such as Platygaster are known to be efficient foragers, whereas larval parasitoids are less efficient. This may create a pattern where larval parasitoid attack saturates more quickly at high host densities, as we observed here. The fact that Platygaster attack rates increased with host density suggest that host densities were relatively low, so we actually may be in the lower part of a type 3 functional response. 

A key question is whether our results would have changed if we had instead manipulated it so that only one of the larval parasitoid species could attack vs. the full complement. Based on the above biology, we may have expected two things. One we would may have seen no selection on oviposition preference in both food webs, since the larval parasitoid tends to have a nonlinear response that actually may enhance genetic variation in oviposition preference. However, we still would have seen selection reduced selection for clutch size, which would free this trait and covarying traits from selective constraints. Also, if we had just included one of the larval parasitoids, which were at lower densities, then the selection gradients would have been weaker simply due to the enhanced survival of gall midge larva. This highlights the importance of studies that manipulate community context in not only looking at the effects on the trait-fitness relationships, but also the effects on average survival. This is especially true in experimental contexts, such as ours, where density of consumers were artificially low, and thus we are only observing a transient dynamic in the strength of selection.


<!--
Selective constraints:

Key Result: More traits under directional selection in simple (3 of 3) vs. complex (1 of 3) food webs. Need to be wary of interpreting no selection on oviposition preference in complex food webs though. Even if there is selection on oviposition preference, this does not change our qualitative result.

Implications: The trajectory of phenotypic evolution is less constrained in complex vs. simple food webs. In other words, there are more potential adaptive peaks in complex food webs. 
--> 

 <!-- This insight also helps resolve the balance between neutral and adaptationist views of phenotypic evolution. In fact, the ability for neutral evolutionary theories to give insight to evolution may be due to the net neutral selection pressures in complex natural systems. However, this highlights that we will need further integration of these perspectives in the context of global change and concomittant changes in the selective environment.-->

 
<!--    
Indirect effects of selection on genetic constraints: 

Key Result: More traits experience a decrease in additive genetic variance and more trait combinations experience selection for trait integration in simple food webs. 

Implications: The structure of the G-matrix is under less influence of selection in complex vs. simple food webs. Thus the trajectory of phenotypic evolution may be dictated more by genetic rather than selective constraints. This is in line with empirical evidence (from complex natural systems) that evolution tends to proceed along genetic lines of least resistance [@Schluter1996].
-->
...
Key Result: Evidence of selection on the extended phenotype of egg parasitoids.

Implications: Hints at a dynamic adaptive landscape and potential indirect evolutionary effects in complex food webs.
-->

 

<!--response to local extinctions will  

Thus, our results hint that indirect evolutionary effects may be common, but latent, in complex communities, and thus local extinctions may trigger their apparent effects.

Understanding these hidden selection pressures in complex communities is important; however, for predicting trajectories of evolutionary change, as they trajectory of evolution will depend on the order of local extinctions or changes in species abundances [Start et al. in press].
-->

<!--
Right now, most studies treat the selection gradients imposed by the adaptive landscape as static [but see @McPeek2017]. Our results suggest though that we also observe indirect evolutionary effects (cite Start, Weis, and Gilbert's new Am Nat paper). This suggests the adaptive landscape itself may be a more dynamic entity in more complex food webs. Where these dynamics end up cancelling each other out in their net selective effects over time, or allow phenotypic evolution to change direction is currently unclear, and we need more empirical work on this front.
-->

<!-- Community context in altering adaptation to environmental change

Relevant review paragraph from @Osmond2017PredatorsHelpPrey about studies incorporating gradual, directional environmental change but also species interactions.

There is a small but growing number of theoretical studies on adaptation to gradual, directional environmental change that incorporate interactions between species (de Mazancourt et al. 2008; Johansson 2008; Jones 2008; Norberg et al. 2012; Mellard et al. 2015). Only two of these studies assessed predator-prey interactions. Jones (2008) used simulations to study, among other things, a predator-prey system where the predator trait evolves to match the prey trait and the prey trait evolves both to match the changing environmental optimum and escape the predator. His main finding was that predation can sometimes increase the persistence time of the prey, presumably when the predator removes prey that are maladapted to the abiotic environment. Mellard et al. (2015) studied an explicit resource-plant-herbivore trophic chain using a primarily numerical approach to ask how the addition of an herbivore affects plant persistence in a warming environment. They analytically clarified the conditions required for the predator to hasten prey evolution but found in simulations that, regardless of the effect on evolutionary rates, herbivory almost never aided plant persistence. In addition, Northfield and Ives (2013), who did not explicitly model a gradually changing environment, analytically demonstrated that conflicting interactions, such as predation, tended to dampen the direct effect of small environmental changes on the equilibrium densities of interacting species, potentially facilitating persistence. In their model, coevolution facilitates persistence through negative feedback loops. For example, as the predator nears extinction, the prey invests less in defense, making it easier for the predator to consume prey and rebound in abundance. Thus, up to this point, while it is clear that predators can theoretically help prey adapt in changing environments, the effect of predators on prey persistence remains unresolved.

-->
<!--
The role of community context and diversity in shaping communities has been explored theoretically in terms of competitive communities [@De_Mazancourt2008; @Johansson2008]. Other work has also highlighted the role of competition in shaping evolutionary rescue [@Osmond2013]. Other studies have also explored the role of predators in helping prey adaptive to environmental change [@Osmond2017PredatorsHelpPrey]. This work has suggested that biodiversity may constrain phenotypic evolution, because more species occupy different niches, preventing the "movement" of species across the adaptive landscape into different ecological niches. This may be true for competitive communities, but in multitrophic communities our empirical results suggest the opposite. Still, in natural communities we are likely to see multiple interaction types that shape the survival of species. The role of different interaction types in shaping evolution in a community context remains both a theoretical and empirical frontier.
-->

<!--
Caveats: 
 
Key Caveat: Experimental artifact of lower larval survival in complex food-web treatment.

Implications: Artificially increased the magnitude of selection in complex food webs. Note however, that this does not qualitatively change any of our results. In fact, we are actually underestimating the constraints imposed by our simple food-web treatment. This is because the egg parasitoids would be released from their intraguild predator, and thus could increase in abundance over future generations and impose stronger selection on gall midges. 
-->
<!--
One of the things that our study did was artificially reduce the mortality induced by parasitoids, in general, in the simpler food web. Importantly though, this would actually make it more difficult to observed selection due to the fact that reductions in mean fitness, all else equal, intensify the strength of selection [@Hunter2018]. Indeed, we are likely underestimating the constraints imposed by simpler food web, because over time, egg parasitoid abundances would increase, thus increasing selective constraints.

 

<!-- Review past experimental studies of complexity -->
<!--There are a growing number of experimental studies examining how the community context shapes the adaptive landscape [@Stinchcombe2001; @Juenger1998; @Lankau2007; @terHorst2010; @Sahli2011; @Lau2012; @terHorst2015; @Schiestl2018]. This body of work focused on testing the extent to which selection imposed by different species is additive or non-additive in a community context [@terHorst2015], a critical step for accurately predicting changes in the trajectory of phenotypic evolution. <!--Put another way, this work gives insight to the direct and indirect effects of selection pressures in a community context. --><!--In contrast, we test how the community context shapes the range of possible evolutionary trajectories (i.e. constraints). We can re-examine the results of this prior work though to see the generality of our results. For example, recent greenhouse experiments with plant-pollinator interactions have shown a tendency for more traits to experience directional selection in simpler vs. more diverse pollinator communities [@Sahli2011; @terHorst2015; @Schiestl2018]. Similarly, @Juenger1998 found that two insect herbviores <!--flies and caterpillars --><!--imposed the strongest reduction in plant fitness in isolation, but these effects were minimized when they were together <!--, highlight a nonadditive interactions that --><!--resulting in neutral selection on flowering time when both were present<!--(unclipped treatment)-->

<!--Our results also suggest an insidious effect of local extinctions. Not only may it compromise community robustness and resilience to future change [@McCann2011], but it may also cause this at the population level. We are in urgent need of future empirical and theoretical work that understands these rippling effects on the robustness of populations and communities to environmental change.-->
...
<!-- <div id="references"></div> this code inserts references wherever you want rather than at the end of the document -->

<!-- CONSIDER ITALICIZING RELATIONSHIPS WHERE 90% CI DOES NOT OVERLAP ZERO -->
...
**Figure \ref{fig:MV_Landscape}**: Two dimensional view of adaptive landscapes of gall midge phenotypes in complex vs. simple food webs. Each panel corresponds to a different combination of phenotypic traits: clutch size and chamber diameter (A); clutch size and oviposition preference (B); oviposition preference and chamber diameter (C). Note the flat landscape in mean larval survival for clutch size and oviposition preference in more complex food webs, whereas all three traits shape the adaptive landscape in simpler food webs. This indicates that selection favors trait integration for each combination of traits in simpler food webs, whereas there is no strong evidence for trait integration in more complex food webs. Note that mean larval survival is plotted on a natural log scale to reflect the mathematical defintion of the adaptive landscape.
