...
```{r Table of differences in food web coefficients}
tidy_foodweb_alphas %>%
  filter(type == "Diff") %>%
...
```{r Table of food web curvatures}
tidy_foodweb_curvs %>%
  filter(type != "Diff") %>%
...
```{r Table of differences in curvatures between food-web treatments}
tidy_foodweb_curvs %>%
  filter(type == "Diff") %>%
...
```{r Tidy up long-term selection gradients, echo=TRUE}
# Calculate differences between food-web treatments, then estimate means and confidence intervals
...
```{r Curvature long-term, echo=TRUE}
# Get mean estimates for directional selection gradients
...
```{r Table of differences in long-term curvatures between food-web treatments}
tidy_foodweb_long_curvs %>%
  filter(type == "Diff") %>%
  select(term, mean, conf.low, conf.high, P_cutoff) %>%
  mutate(mean = round(mean, 2), conf.low = round(conf.low, 2), conf.high = round(conf.high, 2)) %>%
...
```{r Curvature egg vs. larval, echo=TRUE}
# Get mean estimates for directional selection gradients
...
```{r Table of curvatures for egg and larval parasitoids}
tidy_egglarval_curvs %>%
  filter(type != "Diff") %>%
  select(term, type, mean, conf.low, conf.high, P_cutoff) %>% 
  mutate(mean = round(mean, 2), conf.low = round(conf.low, 2), conf.high = round(conf.high, 2)) %>%
  knitr::kable(., caption = "Table of curvatures on gall traits due to egg and larval parasitoids.")
```

```{r Table of differences in curvatures between egg and larval parasitoids}
tidy_egglarval_curvs %>%
  filter(type == "Diff") %>%
  select(term, mean, conf.low, conf.high, P_cutoff) %>%
  mutate(mean = round(mean, 2), conf.low = round(conf.low, 2), conf.high = round(conf.high, 2)) %>%
  knitr::kable(., caption = "Table of differences in curvatures on gall traits between egg and larval parasitoids.")
```
...
AF_uni_Diam <- RF_Diam$absolute_fitness %>% 
  ...
  mutate(ID_group = ifelse(ID == "average", "average", "replicate"),
         Foodweb = factor(ifelse(Foodweb == "Original", "Original", "Removal"), levels = c("Original", "Removal"))) %>% 
...
# Make plots
AF_uni_plots <- plot_grid(AF_uni_Clutch + theme(legend.position = "none"),
                          AF_uni_Pref + theme(legend.position = "none", 
                                           axis.title.y = element_blank()) + xlab("Preference (SDs)"),
                          AF_uni_Diam + theme(legend.position = "none",
...
names(breaks) <- c("Original","Removal")
...
RF_Diam.Clutch$absolute_fitness <- RF_Diam.Clutch$absolute_fitness %>%
  mutate(Foodweb = factor(ifelse(Foodweb == "Original", "Original", "Removal"), levels = c("Original", "Removal")))
...
AF_plot_Diam.Clutch <- RF_Diam.Clutch$absolute_fitness %>%
  ...
  geom_raster(data = filter(RF_Diam.Clutch$absolute_fitness, Foodweb == "Removal"), aes(fill=average)) +
  stat_contour(data = filter(RF_Diam.Clutch$absolute_fitness, Foodweb == "Removal"), aes(z = average), breaks = breaks["Removal"], linetype = "dotted", color = "black") +
...
AF_plot_Diam.Pref <- RF_Diam.Pref$absolute_fitness %>% 
  ...
  geom_raster(aes(fill=average)) +
  stat_contour(aes(z = average), breaks = breaks["Original"], linetype = "dotted", color = "black") +
  geom_raster(data = filter(RF_Diam.Pref$absolute_fitness, Foodweb == "Removal"), aes(fill=average)) +
  stat_contour(data = filter(RF_Diam.Pref$absolute_fitness, Foodweb == "Removal"), aes(z = average), breaks = breaks["Removal"], linetype = "dotted", color = "black") +
  facet_wrap(~Foodweb) + # , labeller = labeller_2D
  xlab("Chamber diameter (SDs)") +
...
AF_plot_Clutch.Pref <- RF_Clutch.Pref$absolute_fitness %>%
  ...
  geom_raster(aes(fill=average)) +
  stat_contour(aes(z = average), breaks = breaks["Original"], linetype = "dotted", color = "black") +
  geom_raster(data = filter(RF_Clutch.Pref$absolute_fitness, Foodweb == "Removal"), aes(fill=average)) +
  stat_contour(data = filter(RF_Clutch.Pref$absolute_fitness, Foodweb == "Removal"), aes(z = average), breaks = breaks["Removal"], linetype = "dotted", color = "black") +
  facet_wrap(~Foodweb) + # , labeller = labeller_2D
  xlab("Oviposition preference (SDs)") +
  ylab("Clutch size (SDs)") +
  scale_x_continuous(labels = c(-1,-0.5,0,0.5,1)) +
...
# Arrow data
gradients_Diam.Clutch <- data.frame(Foodweb = c("Original","Removal"),
                                    sc.Diam = c(0,0), 
                                    sc.log.Clutch = c(0,0), 
...
gradients_Clutch.Pref <- data.frame(Foodweb = c("Original","Removal"),
                                    sc.sqrt.Pref = c(0,0), 
                                    sc.log.Clutch = c(0,0), 
...
gradients_Diam.Pref <- data.frame(Foodweb = c("Original","Removal"),
                                    sc.Diam = c(0,0), 
                                    sc.sqrt.Pref = c(0,0), 
                                    xend = c(filter(tidy_foodweb_grads, type=="Original", term=="sc.Diam")$mean,
                                             filter(tidy_foodweb_grads, type=="Extinction", term=="sc.Diam")$mean), 
...
```{r Curvatures in matrix form, echo=TRUE}
## Original
...
```{r Mean curvature effects on random G-matrices, echo=TRUE, cache=TRUE}
...
We see that for our best estimates of the curvature matrix, that the extinction food web tends to decrease evolvability (`r round(sum(random_evolvabilities$Diff < 0) / length(random_evolvabilities$Diff) * 100,0)`%) of the gall midge population.

```{r Delta evolvability, fig.cap = "Change in evolvability for `r n_Gmats` random G-matrices using our best (mean) estimates of the curvature matrix."}
delta_evolvability_plot <- ggplot(random_evolvabilities, aes(x = Diff)) +
  geom_histogram(alpha = 0.5, binwidth = 0.001) +
  geom_vline(xintercept = 0, linetype = "dotted") +
  xlab(expression(paste(Delta," evolvability (removal - original)"))) +
  ylab("Count") +
  theme_cowplot()
...
```{r Long-term mean curvature effects on random G-matrices, echo=TRUE, cache=TRUE}
...
Over the long-term (after egg parasitoids recover and impose similar fitness impact), we see that for our best estimates of the curvature matrix, that the extinction food web tends to decrease evolvability (`r round(sum(long_random_evolvabilities$Diff < 0) / length(long_random_evolvabilities$Diff) * 100,0)`%) of the gall midge population.

```{r Long-term delta evolvability, fig.cap = "Change in evolvability for 1,000 random G-matrices using our best (mean) estimates of the curvature matrix over the long term."}
...
```{r Mean curvature effects of larval parasitoids on random G-matrices, echo=TRUE, cache=TRUE}
...
We see that for our best estimates of the curvature matrix, that the larval parasitoids tend decrease evolvability in `r round(sum(larvalptoid_random_evolvabilities$Diff < 0) / length(larvalptoid_random_evolvabilities$Diff) * 100,0)`% of the G-matrix scenarios relative to the original food web..


```{r Larval vs Original delta evolvability, fig.cap = "Change in evolvability for 10,000 random G-matrices using our best (mean) estimates of the curvature matrix for larval parasitoids vs original food web."}
larvalptoid_delta_evolvability_plot <- ggplot(larvalptoid_random_evolvabilities, aes(x = Diff)) +
  geom_histogram(alpha = 0.5, binwidth = 0.001) +
  geom_vline(xintercept = 0, linetype = "dotted") +
  xlab(expression(paste(Delta," evolvability (larval parasitoids - original)"))) +
  ylab("Count") +
  theme_cowplot()

larvalptoid_delta_evolvability_plot
save_plot(filename = "larvalptoid_delta_evolvability.pdf", plot = larvalptoid_delta_evolvability_plot, base_height = 4) # base_height = 6 is to large for PDF of manuscript
```
...
for(i in 1:dim(bootstrapped_foodweb_long_curvs)[1]){ # 
  ...
  #                byrow = T, ncol = 3)
  #original_new_G <- Gmat + Gmat %*% original_C %*% Gmat
  #foodweb_original_genetic_constraints[i] <- try(MeanMatrixStatistics(original_new_G, iterations = 10000, parallel = TRUE))["evolvability"]
  ...
  foodweb_long_extinction_genetic_constraints[i] <- try(MeanMatrixStatistics(long_extinction_new_G, iterations = 10000, parallel = TRUE))["evolvability"]
...
eggptoid_selection_plot <- eggptoid_plot_df %>%
  ...
  ylab(expression(paste(Delta," egg parasitoid survival (original - removal)"))) +
  scale_x_continuous(breaks=c(-1,-0.5,0,0.5,1), labels = c(-1,-0.5,0,0.5,1)) +
  geom_hline(yintercept=0, linetype="dotted") +
