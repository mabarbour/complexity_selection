...
This document presents a hybrid summary of the code used to reproduce these analyses. It is intended to give the reader a higher-level picture of the analyses, but does not display more minor code related to aesthetics. The underlying code for this document is provided in *reproduce_analyses.Rmd* and is available at https://github.com/mabarbour/complexity_selection. 
...
Set number of bootstrap replicates for analyses and plots.
...
```{r Multivariate_QQ, fig.show = 'asis', fig.cap="Multivariate quantile-quantile (Q-Q) plot to assess deviations from multivariate normality (black line)."}
mvn_multivariate_qq <- mvn(select(gall_selection.df, sc.Diam, sc.log.Clutch, sc.sqrt.Pref), multivariatePlot = "qq")  
```

\ 

## Selection surfaces on gall midges: food-web treatments
...
tidy_foodweb_alphas %>%
  ...
  mutate(Term = term, Treatment = type, Estimate = round(mean, 3), `2.5%` = round(conf.low, 3), `97.5%` = round(conf.high, 3)) %>%
...
tidy_foodweb_alphas %>%
  ...
  mutate(Estimate = round(mean, 3), `2.5%` = round(conf.low, 3), `97.5%` = round(conf.high, 3)) %>%
 knitr::kable(., caption = "Table of differences in mean fitness and selection surfaces between food-web treatments.")
```
...
## Selection gradients on gall midges: food-web treatments

Using the methods proposed by [Janzen and Stern, 1998](https://onlinelibrary.wiley.com/doi/10.1111/j.1558-5646.1998.tb02237.x), we estimated selection gradients from our selection surfaces.
...
plot_grid(y_title_foodweb_grads, plot_legend_foodweb_grads, ncol=2, rel_widths = c(0.05,1))
...
## Selection surfaces on gall midges: egg vs. larval parasitoids
...
Tidy selection surfaces imposed by egg and larval parasitoids from above models.
...
tidy_egglarval_alphas %>%
  ...
  mutate(mean = round(mean, 3), conf.low = round(conf.low, 3), conf.high = round(conf.high, 3)) %>%   knitr::kable(., caption = "Table of gall selection surfaces imposed by egg and larval parasitoids. Note that type = Simple = Egg parasitoid and Complex = Larval parasitoid.")
```
...
## Selection gradients on gall midges: egg vs. larval parasitoids
...
AF_uni_Diam <- RF_Diam$absolute_fitness %>% 
  ...
  mutate(ID_group = ifelse(ID == "average", "average", "replicate"),
         Foodweb = factor(ifelse(Foodweb == "Complex", "Original", "Extinction"), levels = c("Original", "Extinction"))) %>% 
...
labels_2D <- list("Complex"="Original", "Simple"="Extinction")
...
## Selection surfaces of the egg parasitoid *Platygaster* sp.
...
Use parametric bootstrapping to calculating 95% confidence intervals of selection surfaces.
...
Remove all higher-order terms to quantify linear selection surfaces.
...
Gather regression coefficients to display selection surfaces.
...
```{r Egg ptoid coefficients in each food-web treatment}
tidy_eggegg_alphas %>%
  filter(type != "Diff") %>%
  select(term, type, mean, conf.low, conf.high) %>% # coefficient_type, P_cutoff
  mutate(mean = round(mean, 3), conf.low = round(conf.low, 3), conf.high = round(conf.high, 3)) %>%
...
```{r Table of differences in egg parasitoid coefficients between food-web treatments, include=FALSE}
tidy_eggegg_alphas %>%
  filter(type == "Diff") %>%
  select(term, mean, conf.low, conf.high) %>% # coefficient_type, P_cutoff
  mutate(mean = round(mean, 3), conf.low = round(conf.low, 3), conf.high = round(conf.high, 3)) %>%
...
## Selection gradients acting on the egg parasitoid
...
```{r Egg vs. ptoid selection gradients}
tidy_eggegg_grads %>%
  filter(type != "Diff") %>%
  select(term, type, mean, conf.low, conf.high) %>% # gradient_type, P_cutoff
  mutate(mean = round(mean, 3), conf.low = round(conf.low, 3), conf.high = round(conf.high, 3)) %>%
knitr::kable(., caption = "Table of selection gradients acting on egg parasitoids in complex and simple food webs.")
```

\  

```{r Table of differences in selection gradients acting on egg parasitoids between food-web treatment, include=FALSE}
tidy_eggegg_grads %>%
  filter(type == "Diff") %>%
  select(term, mean, conf.low, conf.high) %>% # gradient_type, P_cutoff
  mutate(mean = round(mean, 3), conf.low = round(conf.low, 3), conf.high = round(conf.high, 3)) %>%
