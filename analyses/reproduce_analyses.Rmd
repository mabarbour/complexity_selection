...
```{r Bootstrap food-web model, cache=TRUE, include=FALSE}
...
```{r Bootstrap linear food-web model, cache=TRUE, include=FALSE}
...
```{r Food-web alpha coefficients, include=FALSE}
...
```{r Table of food web coefficients, echo=F}
knitr::kable(tidy_foodweb_alphas)
```


```{r Plot food-web coefficients, echo=FALSE}
...
```{r Food-web selection gradients, include=FALSE}
...
```{r Plot food-web gradients, echo=FALSE}
...
```{r Bootstrap egg vs. larval ptoid model, cache=TRUE, include=FALSE}
...
```{r Tabel of egg vs. larval coefficients, echo=FALSE}
knitr::kable(tidy_egglarval_alphas)
```


```{r Plot egglarval coefficients, echo=FALSE}
...
```{r Egg vs. larval ptoid selection gradients, include=FALSE}
...
# Get legend
#legend_foodwebegglarval_grads <- get_legend(plot_foodwebegglarval_directional_grads)
...
#  nrow=2, align="hv", labels = "")
#plot_legend_foodwebegglarval_grads <- ggdraw(plot_foodwebegglarval_grads) + draw_grob(legend_foodwebegglarval_grads, x = 0.7, y=-0.2)
...
```{r Plot foodwebegglarval coefficients, echo=FALSE, fig.cap="**Partitioning Selection Gradients.**"}
...
plot_foodwebegglarval_mean_fitness <- tidy_foodwebegglarval_alphas %>%
  ...
  geom_linerange(aes(ymax=conf.high, ymin=conf.low, color=type), position=dodge_width) +
  geom_point(aes(y=mean, fill=type), position=dodge_width, shape=21, color="black", size=point.size) +
 # geom_text(data=filter(tidy_foodwebegglarval_alphas, type=="Diff", term=="(Intercept)"), aes(y=0.9, x=1.5, label=P_cutoff), size=P_size, color="black") +
  scale_y_continuous(name="larval survival", limits=c(0,1), breaks=c(0,0.2,0.4,0.6,0.8,1)) +
...
plot_foodwebegglarval_linear_coefs <- tidy_foodwebegglarval_alphas %>%
  ...
  geom_linerange(aes(ymax=conf.high, ymin=conf.low, color=type), position=dodge_width) +
  geom_point(aes(y=mean, fill=type), position=dodge_width, shape=21, color="black", size=point.size) +
  #geom_text(data=filter(tidy_foodwebegglarval_alphas, type=="Diff", term %in% c("sc.Diam","sc.log.Clutch","sc.sqrt.Pref")), 
  #          aes(y=P_asterisk_foodwebegglarval_coefs, label=P_cutoff), size=P_size, color="black") +
...
plot_foodwebegglarval_quadratic_coefs <- tidy_foodwebegglarval_alphas %>%
  ...
  geom_linerange(aes(ymax=conf.high, ymin=conf.low, color=type), position=dodge_width) +
  geom_point(aes(y=mean, fill=type), position=dodge_width, shape=21, color="black", size=point.size) +
  #geom_text(data=filter(tidy_foodwebegglarval_alphas, type=="Diff", term %in% c("I(sc.Diam^2)","I(sc.log.Clutch^2)","I(sc.sqrt.Pref^2)")), 
  #          aes(y=P_asterisk_foodwebegglarval_coefs, label=P_cutoff), size=P_size, color="black") +
  scale_y_continuous(name="Coefficient", limits=y_limits_foodwebegglarval_coefs) +
  scale_x_discrete(name="", labels=parse(text=c("alpha[Diam:Diam]","alpha[Clutch:Clutch]","alpha[Pref:Pref]"))) +
  facet_wrap(~coefficient_type) +
...
plot_foodwebegglarval_correlational_coefs <- tidy_foodwebegglarval_alphas %>%
  ...
  geom_linerange(aes(ymax=conf.high, ymin=conf.low, color=type), position=dodge_width) +
  geom_point(aes(y=mean, fill=type), position=dodge_width, shape=21, color="black", size=point.size) +
 # geom_text(data=filter(tidy_foodwebegglarval_alphas, type=="Diff", term %in% c("sc.Diam:sc.log.Clutch","sc.Diam:sc.sqrt.Pref","sc.log.Clutch:sc.sqrt.Pref")), 
  #          aes(y=P_asterisk_foodwebegglarval_coefs, label=P_cutoff), size=P_size, color="black") +
  scale_y_continuous(name="Coefficient", limits=y_limits_foodwebegglarval_coefs) +
  scale_x_discrete(name="", labels=parse(text=c("alpha[Diam:Clutch]","alpha[Diam:Pref]","alpha[Clutch:Pref]"))) +
  facet_wrap(~coefficient_type) +
...
```{r simulate bias in chamber diameter, include=FALSE, eval=FALSE}

# To determine whether I should adjust any coefficients associated with chamber diameter, e.g. quadratic and correlation terms, I conducted a quick simulation. In the simulation, I assumed that if the larva survive, then the chamber diameter was the corrent one. However, if the larva did not survive, its size could be due to both its size and parasitism. I assumed a constant value of parasitism effect on gall size.

n <- 10000
diam <- rnorm(n, mean=0, sd=1) # simulate 1,000 chamber diameters from a normal distribution with mean=0 and sd=1
pref <- rnorm(n, mean=0, sd=1)               # simulate 1,000 Oviposition preferences from a normal distribution with mean=0 and sd=1
bias.adj <- 0.34                  # this value corresponds to actual estimates from my bias analyses.
mean_survival <- 0.5              # approximate survival across food-web treatments
logit_survival <- mean_survival + 1.5*diam + 0.1*diam*diam -0.2*pref + 0*pref*pref -0.2*diam*pref # using coefficients that closely match empirical estimates
link_survival <- exp(logit_survival)/(1 + exp(logit_survival))
survival <- (runif(n) < link_survival) 

sim_df <- data.frame(survival=survival, diam=diam, pref=pref) %>%
  mutate(diam_adj = ifelse(survival==FALSE, diam + bias.adj, diam)) # adjust bias only if larva did not survive

## Fit non-adjusted model
nonadj_model <- glm(survival ~ diam + I(diam^2) + pref + I(pref^2) + diam*pref, data=sim_df, family=binomial) 
summary(nonadj_model)

adj_model <- glm(survival ~ diam_adj  + I(diam_adj^2) + pref + I(pref^2) + diam_adj*pref, data=sim_df, family=binomial) 
summary(adj_model)

## Interesting, it seems that my simulation design affects the intercept term; however, its clear that my biological hypothesis for how chamber diameter is influenced by parasitism (simply decreasing size of non-surviving larva), would not affect the quadratic or nonlinear terms.
data.frame(terms = names(coef(nonadj_model)), nonadj_coefs = coef(nonadj_model), adj_coefs = coef(adj_model)) %>%
  mutate(diff_coefs = nonadj_coefs - adj_coefs)
```
...
```{r Get Oviposition preference Fitness Landscape, cache=TRUE, include=FALSE}
...
# Get legend
AF_uni_legend <- get_legend(AF_uni_Diam + theme(legend.title.align = 0.5)) #get_legend(AF_uni_Diam)
...
AF_uni_plots <- plot_grid(AF_uni_Diam + theme(legend.position = "none"), 
                      ...
                      AF_uni_Clutch + theme(legend.position = "none",
                                            ...
                                            # axis.text.x = element_text(size=10),
                                            # axis.title.x = element_text(size=11)),
                    # legend_only + theme(legend.position = "none",                          
                    #                         axis.title.y = element_blank(), 
                    #                         axis.text.x = element_text(size=10),
                    #                         axis.title.x = element_text(size=11)),
...
```{r Figure-Univariate-Landscapes, echo=FALSE, eval=FALSE}
# Get legend
AF_uni_legend <- get_legend(legend_only + theme(legend.position="top", legend.title.align = 0.5) + guides(color=guide_legend(title.position="top")))


# Make plots
AF_uni_plots <- plot_grid(NULL,
                          Diam_legend_only + theme(legend.position = "none",                         
                                             axis.title.y = element_blank(),
                                             axis.text.x = element_blank()),
                     pref_legend_only + theme(legend.position = "none",                          
                                             axis.title.y = element_blank(),
                                             axis.text.x = element_blank()) + xlab("Preference (SDs)"),
                     legend_only + theme(legend.position = "none",                         
                                             axis.title.y = element_blank()),
                       labels = c("","A", "B", "C"), hjust=0.06, vjust=0.1, 
                     ncol = 1, nrow=4, align='hv', rel_heights = c(0.5,1,1,1))

AF_uni_legend_plots <- ggdraw(AF_uni_plots) + draw_grob(AF_uni_legend, x=0.15,y=0.4)
y_title_AF_uni_plots <- ggdraw() + draw_label("Mean larval survival", angle = 90, size=16)

AF_gradients <- plot_grid(y_title_AF_uni_plots, AF_uni_legend_plots, ncol=2, rel_widths = c(0.1,1))
AF_gradients
save_plot(filename = "UV_landscapes.pdf", plot = AF_gradients, base_height = 9, base_width = 7)
```

```{r Alt Figure-Univariate-Landscapes, echo=FALSE, eval=FALSE}
# Get legend
AF_uni_legend <- get_legend(legend_only + theme(legend.title.align = 0.5))

# Make plots
AF_uni_plots <- plot_grid(NULL,
                          Diam_legend_only + theme(legend.position = "none",                          # AF_uni_egglarval_Clutch
                                             axis.title.y = element_blank()), 
                                             #axis.text.y = element_blank(),
                                             #axis.text.x = element_text(size=10),
                                             #axis.title.x = element_text(size=11)),
                     pref_legend_only + theme(legend.position = "none",                          # AF_uni_egglarval_Clutch
                                             axis.title.y = element_blank()), 
                                             #axis.text.y = element_blank(),
                                             #axis.text.x = element_text(size=10),
                                             #axis.title.x = element_text(size=11)),
                     legend_only + theme(legend.position = "none",                          # AF_uni_egglarval_Clutch
                                             axis.title.y = element_blank()), 
                                             #axis.text.y = element_blank(),
                                             #axis.text.x = element_text(size=10),
                                             #axis.title.x = element_text(size=11)),
...
AF_uni_legend_plots <- ggdraw(AF_uni_plots) + draw_grob(AF_uni_legend, x=0.1, y=0.25) #ggdraw(AF_uni_plots) + draw_grob(AF_uni_legend, x = 0.7, y=-0.2) # draw_grob(AF_uni_legend, x = 0.8, y=0)
y_title_AF_uni_plots <- ggdraw() + draw_label("Mean larval survival", angle = 90)
AF_gradients <- plot_grid(y_title_AF_uni_plots, AF_uni_legend_plots, ncol=2, rel_widths = c(0.05,1))
AF_gradients
#AF_gradients <- plot_grid(AF_uni_legend, AF_uni_plots,  nrow = 2, rel_heights = c(0.1,1)) #ggdraw(AF_uni_plots) + draw_grob(AF_uni_legend, x = 0.7, y=-0.2) # draw_grob(AF_uni_legend, x = 0.8, y=0)
#AF_gradients

```

```{r Egg-Parasitoid-Selection, echo=F}
...
save_plot(filename = "selection_on_Platygaster.pdf", plot = eggptoid_selection_plot, base_height = 4) # base_height = 6
```

```{r Egg-Parasitoid-Selection-linear, echo=F}
linear_eggptoid_plot_df <- linear_eggegg_RF_Pref$absolute_fitness %>% 
  select(-sc.Diam, -sc.log.Clutch) %>%
  gather(ID, absolute_fitness, -sc.sqrt.Pref, -Foodweb) %>%
  mutate(ID_group = ifelse(ID == "average", "average", "replicate")) %>%
  spread(Foodweb, absolute_fitness) %>%
...
linear_eggptoid_selection_plot <- linear_eggptoid_plot_df %>%
  ggplot(., aes(x=sc.sqrt.Pref, y=fitness_difference, group=ID, alpha=ID_group, size=ID_group)) +
  geom_line(color=cbPalette[5]) +
  scale_alpha_manual(values=c(1,0.2), guide=FALSE) +
  scale_size_manual(values=c(1.5,0.25), guide=FALSE) +
...
linear_eggptoid_selection_plot
save_plot(filename = "linear_selection_on_Platygaster.pdf", plot = linear_eggptoid_selection_plot, base_height = 6)
```


```{r Plot Univariate RF Landscapes, include=FALSE}
...
RF_Diam_legend_only <- bind_rows(RF_plot_foodweb_Diam_df, RF_plot_egglarval_Diam_df) %>%
  ...
  xlab("Chamber diameter (SDs)") +
  ylab("Relative fitness") +
  scale_color_manual(values = cbPalette[c(6,5,4)], name = "Parasitoids") + # "Food web"
  scale_x_continuous(breaks=c(-1,-0.5,0,0.5,1), labels = c(-1,-0.5,0,0.5,1)) +
  coord_cartesian(ylim=c(0.2,2)) +
  theme(legend.title.align = 0.5) +
  geom_hline(yintercept=1, linetype="dotted")
...
# Get legend
RF_uni_legend <- get_legend(RF_legend_only) #  + theme(legend.position = "top") 

# Make plots
RF_uni_plots <- plot_grid(RF_Diam_legend_only + theme(legend.position = "none",                          # AF_uni_egglarval_Clutch
                                             #axis.title.y = element_blank(), 
                                             #axis.text.y = element_blank(),
                                             axis.text.x = element_text(size=10),
                                             axis.title.x = element_text(size=11)),
                     RF_pref_legend_only + theme(legend.position = "none",                          # AF_uni_egglarval_Clutch
                                             axis.title.y = element_blank(), 
                                             #axis.text.y = element_blank(),
                                             axis.text.x = element_text(size=10),
                                             axis.title.x = element_text(size=11)),
                     RF_legend_only + theme(legend.position = "none",                          # AF_uni_egglarval_Clutch
                                             axis.title.y = element_blank(), 
                                             #axis.text.y = element_blank(),
                                             axis.text.x = element_text(size=10),
                                             axis.title.x = element_text(size=11)),
...
```{r Get Clutch Size x Oviposition preference Landscape, cache=TRUE, include=FALSE}
...
```{r Get Gall Diameter x Oviposition preference Landscape, cache=TRUE, include=FALSE}
...
```{r Get plots of multivariate adaptive landscape, include=FALSE}
...
AF_landscape_plots <- plot_grid(
  AF_plot_Diam.Clutch + theme(legend.position = "none", 
                             ...
                             axis.text.x = element_blank()) +
    #stat_contour(aes(z = average), breaks = breaks, linetype = "dotted", color = "black") + # mean(RF_Diam.Pref$absolute_fitness$average)
  ...
  AF_plot_Clutch.Pref + theme(legend.position = "none", 
    ...
    #stat_contour(aes(z = average), breaks = breaks, linetype = "dotted", color = "black") +
...
Remove 2nd contour line

```{r Figure-Multivariate-Landscapes-2, echo=FALSE, fig.cap="Fitness landscapes of gall traits in complex vs. simple food webs. Each panel corresponds to a different combination of traits: clutch size and gall diameter (A); clutch size and Oviposition preference (B); Oviposition preference and gall diameter (C). Note that traits for all plots range 1 SD below and above the mean (=0)."}
...
AF_landscape_2d <- ggdraw(AF_landscape_plots) + draw_grob(AF_landscape_legend, x = 0.7, y=-0.2)
AF_landscape_2d
save_plot(filename = "MV_landscapes_2.pdf", plot = AF_landscape_2d, base_width=8, base_height = 6)
```
...
```{r Model 1, cache=TRUE, eval=FALSE, include=FALSE}
model_1 <- glmer(
...
We use parametric bootstrapping (n=`r n_boots_analysis`) to evaluate the statistical significance of the effect of food-web treatment on nonlinear terms (i.e. Foodweb:trait~i~^2^ or Foodweb:trait~i~:trait~j~). Food-web treatment does influence the quadratic effect of of Oviposition preference on larval survival. None of the other terms statistically differ from zero.

```{r Bootstrap Model 1, cache=TRUE, include=FALSE, eval=FALSE}
...
```{r Plot Effect of Food-web on Nonlinear Terms in Model 1, echo=FALSE, eval=FALSE, include=FALSE}
...
```{r Model 2, cache=TRUE, eval=FALSE, include=FALSE}
...
```{r Bootstrap Model 2, cache=TRUE, include=FALSE, eval=FALSE, include=FALSE}
...
```{r Plot Nonlinear Terms, echo=FALSE, eval=FALSE, include=FALSE}
...
```{r Adjust Model 2 for Slopes, eval=FALSE, include=FALSE}
model_2_adj <- update(model_2, .~. -I(sc.sqrt.Pref^2))
```


```{r Boot Adjusted Model 2 for accurate slope estimates, cache=TRUE, eval=FALSE, include=FALSE}
...
```{r Plot Nonlinear Terms on appropriate scale, echo=FALSE, eval=FALSE, include=FALSE}
...
```{r Fit Model 3, cache=TRUE, eval=FALSE, include=FALSE}
...
```{r Bootstrap Model 3, cache=TRUE, include=FALSE, eval=FALSE}
...
```{r Plot Model 3, echo=FALSE, eval=FALSE, include=FALSE}
...
```{r Fit Model 4, cache=TRUE, eval=FALSE, include=FALSE}
...
```{r Bootstrap Model 4, cache=TRUE, include=FALSE, eval=FALSE, include=FALSE}
...
```{r Plot Model 4, echo=FALSE, eval=FALSE, include=FALSE}
...
```{r Fit Model 4 adjusted for estimating clutch slopes, cache=TRUE, eval=FALSE, include=FALSE}
model_4_adj <- update(model_4, .~. -sc.log.Clutch)
```

```{r Bootstrap adjusted model 4, cache=TRUE, include=FALSE, eval=FALSE}
...
```{r Plot adjusted model 4, echo=FALSE, eval=FALSE, include=FALSE}
...
```{r Average larval survival, cache=TRUE, eval=FALSE, include=FALSE}
...
```{r Organize and Transform Coefficients, include=FALSE, eval=FALSE}
...
```{r Calculate Selection Gradients, include=FALSE, eval=FALSE}
...
```{r Table of Selection Gradients, echo=FALSE, eval=FALSE, include=FALSE}
...
```{r Plot of Selection Gradients, echo=FALSE, eval=FALSE, include=FALSE}
selection_gradients %>%
  ggplot(., aes(x=term_ord)) +
...
```{r Egg vs. Larval Parasitoids Model, cache=TRUE, eval=FALSE, include=FALSE}
...
```{r Bootstrap Egg vs. Larval Parasitoid Model 2, cache=TRUE, include=FALSE, eval=FALSE}
...
```{r Plot effect of Egg vs. Larval Parasitoid Model 2, echo=FALSE, eval=FALSE, include=FALSE}
...
```{r Egg vs. Larval Parasitoids adjusted model 2, cache=TRUE, eval=FALSE, include=FALSE}
...
```{r Plot effect of Egg vs. Larval Parasitoid adjusted Model 2, echo=FALSE, eval=FALSE, include=FALSE}
...
```{r Fit Egg vs. Larval Parasitoid Model 4, cache=TRUE, eval=FALSE, include=FALSE}
egg.larval_model_4 <- update(model_4, data=egg.vs.larval_df)
```

This reduced model returns the same qualitative result as before. Specifically, that the linear effect of clutch size depends on food-web treatment. Note also that larval survival is significantly higher in the simple food web compared to the complex food web (dotted line). These are the regression coefficients we will use when calculating directional selection gradients ($\beta$).

```{r Bootstrap Egg vs. Larval Parasitoid Model 4, cache=TRUE, include=FALSE, eval=FALSE}
...
```{r Plot Egg vs. Larval Parasitoid Model 4, echo=FALSE, eval=FALSE, include=FALSE}
...
```{r Fit Egg vs. Larval Parasitoid adjusted Model 4, cache=TRUE, eval=FALSE, include=FALSE}
egg.larval_model_4_adj <- update(model_4_adj, data=egg.vs.larval_df)
```

This reduced model returns the same qualitative result as before. Specifically, that the linear effect of clutch size depends on food-web treatment. Note also that larval survival is significantly higher in the simple food web compared to the complex food web (dotted line). These are the regression coefficients we will use when calculating directional selection gradients ($\beta$).

```{r Bootstrap Egg vs. Larval Parasitoid adjuted Model 4, cache=TRUE, include=FALSE, eval=FALSE}
...
Here, we observe clear evidence of conflicting selection pressures on clutch size. This results in net zero selection gradient acting on clutch size in the complex food web.
```{r Plot Egg vs. Larval Parasitoid adjusted Model 4, echo=FALSE, eval=FALSE, include=FALSE}
...
```{r Organize and Transform egg vs. larval parasitoid Coefficients, include=FALSE, eval=FALSE}
...
```{r Calculate Egg vs. larval parasitoid Selection Gradients, include=FALSE, eval=FALSE}
...
```{r Table of Egg vs. Larval Parasitoid Selection Gradients, echo=FALSE, eval=FALSE, include=FALSE}
...
```{r Plot of Egg vs. Larval Parasitoid Selection Gradients, echo=FALSE, eval=FALSE, include=FALSE}
...
```{r New approach for biased selection, include=FALSE, eval=FALSE}
...
```{r Redo selection gradients, include=FALSE, eval=FALSE}
...
```{r Bias in gradients on chamber diameter, eval=FALSE, include=FALSE}
fixef(biased.diam_model_4)["sc.Diam"]
```

```{r Variance explained in chamber diameter, eval=FALSE, include=FALSE}
...
```{r Variance explained in other traits, eval=FALSE, include=FALSE}
...
```{r female pref plot, echo=F, eval=FALSE, include=FALSE}
visreg::visreg(female_pref, xvar="Foodweb")
```


```{r Plot of bias adjusted Selection Gradients, echo=FALSE, eval=FALSE, include=FALSE}
...
```{r Prep selection gradients figure, echo=FALSE, eval=FALSE, include=FALSE}
...
```{r Selection-Gradients, echo=FALSE, fig.cap="Selection gradients. Points and lines represents means and 95% confidence intervals (n=1,000 bootstrapped samples).", eval=FALSE, include=FALSE}
...
```{r Selection on Platygaster Traits, cache=TRUE, eval=FALSE, include=FALSE}
...
```{r Plot models of Platygaster selection differentials, echo=FALSE, eval=FALSE, include=FALSE}
...
visreg_platy_Pref <- visreg(platy_Pref, xvar="Foodweb", gg=TRUE) + ylab("Oviposition preference (SD)") + xlab("")
...
```{r Table of Platygaster selection differentials, echo=FALSE, eval=FALSE, include=FALSE}
...
```{r Observe P matrix, include=FALSE, eval=FALSE}
...
```{r Change in G-matrices, include=FALSE, eval=FALSE}
...
In simple food webs, both clutch size and Oviposition preference experience directional selection, with smaller clutch sizes ($\beta_{clutch}=$
...
`r paste0(filter(summary_beta_treatment, Phenotype=="Oviposition preference")$Estimate," [",filter(summary_beta_treatment, Phenotype=="Oviposition preference")$lower_2.5,",",filter(summary_beta_treatment, Phenotype=="Oviposition preference")$upper_97.5,"]")`
...
) or Oviposition preference ($\beta_{pref}=$
`r paste0(filter(summary_beta_control, Phenotype=="Oviposition preference")$Estimate," [",filter(summary_beta_control, Phenotype=="Oviposition preference")$lower_2.5,",",filter(summary_beta_control, Phenotype=="Oviposition preference")$upper_97.5,"]")`
...
```{r Egg vs. egg Parasitoids Model, cache=TRUE, eval=FALSE, include=FALSE}
...
```{r egg vs egg ggplot explore, eval=FALSE, include=FALSE}
...
```{r Egg vs. Egg Bootstrap Model 1, include=FALSE, cache=TRUE, eval=FALSE}
...
```{r Egg vs. Egg Plot Effect of Food-web on Nonlinear Terms in Model 1, echo=FALSE, eval=FALSE, include=FALSE}
...
```{r Fit Egg vs. egg Parasitoid Model 3, cache=TRUE, eval=FALSE, include=FALSE}
egg.egg_model_3 <- update(model_3, data=egg.vs.egg_df) 
```


```{r Bootstrap Egg vs. egg Parasitoid Model 4, cache=TRUE, include=FALSE, eval=FALSE}
...
Here, we observe clear evidence of conflicting selection pressures on clutch size. This results in net zero selection gradient acting on clutch size in the complex food web.
```{r Plot Egg vs. egg Parasitoid Model 4, echo=FALSE, eval=FALSE, include=FALSE}
...
```{r Fit Egg vs. egg Parasitoid adjusted Model 4, cache=TRUE, eval=FALSE, include=FALSE}
...
Here, we observe clear evidence of conflicting selection pressures on clutch size. This results in net zero selection gradient acting on clutch size in the complex food web.
```{r Plot Egg vs. egg Parasitoid adjusted Model 4, echo=FALSE, eval=FALSE, include=FALSE}
...
```{r Table of Egg vs. Egg Parasitoid Selection Gradients, echo=FALSE, eval=FALSE, include=FALSE}
egg.larval_selection_gradients$term_ord <- factor(egg.larval_selection_gradients$term, 
                                   levels=c("sc.Diam","sc.log.Clutch","sc.sqrt.Pref","I(sc.Diam^2)","I(sc.log.Clutch^2)","I(sc.sqrt.Pref^2)","sc.Diam:sc.log.Clutch","sc.Diam:sc.sqrt.Pref","sc.log.Clutch:sc.sqrt.Pref"),
                                   labels=c("Diam","Clutch","Pref","Diam^2","Clutch^2","Pref^2","Diam:Clutch","Diam:Pref","Clutch:Pref"),
                                   ordered = TRUE)
...
```{r Plot of Egg vs. Egg Parasitoid Selection Gradients, echo=FALSE, eval=FALSE, include=FALSE}
egg.larval_selection_gradients %>%
  ggplot(., aes(x=term_ord)) +
  geom_linerange(aes(ymin=`2.5%`, ymax=`97.5%`, color=`Food web`), position=position_dodge(width=0.3)) +
  geom_point(aes(y=gradient, fill=`Food web`), color="black", shape=21, size=3, position=position_dodge(width=0.3)) +
