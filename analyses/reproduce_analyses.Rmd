...
# all sources of mortality
mean(full_df$gall_survival) + mean(full_df$egg.ptoid) + mean(full_df$larval.ptoid) + mean(full_df$early.larval.death) + mean(full_df$unknown_mortality)

mean(full_df$gall_survival) # 40% survival
mean(full_df$egg.ptoid) # 26% egg parasitism
mean(full_df$larval.ptoid) # 11% larval parasitism
mean(full_df$early.larval.death) # 17% early larval death
mean(full_df$unknown_mortality) # 7% unknown
...
biased_foodweb_confint %>%
  ...
  filter(term %in% c("FoodwebComplex:sc.Diam","FoodwebSimple:sc.Diam")) %>%
...
foodweb_complex_raw_curvs <- foodweb_complex_raw_gradients %>%
  transmute(`FoodwebComplex:C.Diam` = `FoodwebComplex:I(sc.Diam^2)` - `FoodwebComplex:sc.Diam`*`FoodwebComplex:sc.Diam`,
         `FoodwebComplex:C.Clutch` = `FoodwebComplex:I(sc.log.Clutch^2)` - `FoodwebComplex:sc.log.Clutch`*`FoodwebComplex:sc.log.Clutch`,
         `FoodwebComplex:C.Pref` = `FoodwebComplex:I(sc.sqrt.Pref^2)` - `FoodwebComplex:sc.sqrt.Pref`*`FoodwebComplex:sc.sqrt.Pref`,
         `FoodwebComplex:C.Diam:Clutch` = `FoodwebComplex:sc.Diam:sc.log.Clutch` - `FoodwebComplex:sc.Diam`*`FoodwebComplex:sc.log.Clutch`,
         `FoodwebComplex:C.Diam:Pref` = `FoodwebComplex:sc.Diam:sc.sqrt.Pref` - `FoodwebComplex:sc.Diam`*`FoodwebComplex:sc.sqrt.Pref`,
         `FoodwebComplex:C.Clutch:Pref` = `FoodwebComplex:sc.log.Clutch:sc.sqrt.Pref` - `FoodwebComplex:sc.log.Clutch`*`FoodwebComplex:sc.sqrt.Pref`)

# Calculate curvatures for simple food web
foodweb_simple_raw_curvs <- foodweb_simple_raw_gradients %>%
  transmute(`FoodwebSimple:C.Diam` = `FoodwebSimple:I(sc.Diam^2)` - `FoodwebSimple:sc.Diam`*`FoodwebSimple:sc.Diam`,
         `FoodwebSimple:C.Clutch` = `FoodwebSimple:I(sc.log.Clutch^2)` - `FoodwebSimple:sc.log.Clutch`*`FoodwebSimple:sc.log.Clutch`,
         `FoodwebSimple:C.Pref` = `FoodwebSimple:I(sc.sqrt.Pref^2)` - `FoodwebSimple:sc.sqrt.Pref`*`FoodwebSimple:sc.sqrt.Pref`,
         `FoodwebSimple:C.Diam:Clutch` = `FoodwebSimple:sc.Diam:sc.log.Clutch` - `FoodwebSimple:sc.Diam`*`FoodwebSimple:sc.log.Clutch`,
         `FoodwebSimple:C.Diam:Pref` = `FoodwebSimple:sc.Diam:sc.sqrt.Pref` - `FoodwebSimple:sc.Diam`*`FoodwebSimple:sc.sqrt.Pref`,
         `FoodwebSimple:C.Clutch:Pref` = `FoodwebSimple:sc.log.Clutch:sc.sqrt.Pref` - `FoodwebSimple:sc.log.Clutch`*`FoodwebSimple:sc.sqrt.Pref`)
...
AF_uni_Diam <- RF_Diam$absolute_fitness %>% 
  ...
  mutate(ID_group = ifelse(ID == "average", "average", "replicate"),
         Foodweb = factor(ifelse(Foodweb == "Complex", "Intact", "Extinction"), levels = c("Intact", "Extinction"))) %>% 
...
labels_2D <- list("Complex"="Intact", "Simple"="Extinction")
...
```{r Bootstrap linear egg vs. egg model, cache=TRUE}
...
eggptoid_selection_plot <- eggptoid_plot_df %>%
  ...
  ylab(expression(paste(Delta," egg parasitoid survival (intact - extinction)"))) +
  scale_x_continuous(breaks=c(-1,-0.5,0,0.5,1), labels = c(-1,-0.5,0,0.5,1)) +
  geom_hline(yintercept=0, linetype="dotted") +
