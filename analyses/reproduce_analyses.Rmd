...
```{r setup}
knitr::opts_chunk$set(messages=FALSE, warning=FALSE, error=FALSE)
...
gall_selection.df <- read_csv("../data/gall_selection_manuscript_dataset.csv") %>%
  ...
  filter(#phenology == "early", Location == "tree",
         egg.ptoid > 0 | larval.ptoid > 0 | pupa > 0) %>%                  # eliminate unknown sources of mortality
...
```{r Plot foodwebegglarval coefficients, fig.cap="**Partitioning Selection Gradients.**", include=FALSE}
