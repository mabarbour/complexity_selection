...
mvn_assumption <- mvn(select(gall_selection.df, sc.Diam, sc.log.Clutch, sc.log1p.Pref), multivariatePlot = "qq", univariatePlot = "histogram")

# mvn(select(gall_selection.df, sc.Diam, sc.log.Clutch, sc.log1p.Pref), multivariatePlot = "qq")
...
```{r Boot Adjusted Model 2 for accurate slope estimates, cache=TRUE}
...
Note that there was no effect of food-web structure on on chamber diameter (coef=`r round(filter(tidy_model_3, term=="FoodwebSimple:sc.Diam")$statistic,2)`, 95%CI=`r round(filter(tidy_model_3, term=="FoodwebSimple:sc.Diam")$conf.low,2)`,`r round(filter(tidy_model_3, term=="FoodwebSimple:sc.Diam")$conf.high,2)`) or female preference (coef=`r round(filter(tidy_model_3, term=="FoodwebSimple:sc.log1p.Pref")$statistic,2)`, 95%CI=`r round(filter(tidy_model_3, term=="FoodwebSimple:sc.log1p.Pref")$conf.low,2)`,`r round(filter(tidy_model_3, term=="FoodwebSimple:sc.log1p.Pref")$conf.high,2)`).
...
tidy_model_4_survival_ests <- tidy(boot_model_4_survival_ests, conf.int=T) 

tidy_model_4_survival_ests
```
...
```{r Plot effect of Egg vs. Larval Parasitoid adjusted Model 2, echo=FALSE}
...
female_pref <- lmer(sc.log1p.Pref ~ Foodweb + (1|Genotype), data=distinct(gall_selection.df, Foodweb, Genotype, Plant_Position, sc.log1p.Pref))
summary(female_pref)
...
quadratic <- filter(focus_gradients, Type=="Quadratic") %>%
  ...
  geom_hline(yintercept=0, linetype="dotted") +
  ylab("Selection Gradient") +
  scale_fill_manual(values=cbPalette[c(6,4,5)]) +
  scale_color_manual(values=cbPalette[c(6,4,5)]) +
  facet_wrap(~Type_ord, ncol=2, scales="free_x") +
...
platy_Pref_levene <- leveneTest(sc.log1p.Pref ~ Foodweb, platy_selection_df)
```
...
```{r Calculate Selection Differentials for Platygaster, cache=TRUE, include=FALSE}
...
platy_quadratic_selection <- platy_selection_df %>%
  group_by(Foodweb) %>%
  summarise_at(c("sc.Diam","sc.log.Clutch","sc.log1p.Pref"), var) %>%
...
```{r Table of Platygaster selection differentials, echo=FALSE}
...
AF_plot_Diam.Pref <- RF_Diam.Pref$absolute_fitness %>% 
  ggplot(., aes(x=sc.Diam, y=sc.log1p.Pref)) +
  geom_raster(aes(fill=average)) +
  facet_wrap(~Foodweb) +
  xlab("Chamber diameter (SDs)") +
...
AF_plot_Clutch.Pref <- RF_Clutch.Pref$absolute_fitness %>%
  ggplot(., aes(x=sc.log1p.Pref, y=sc.log.Clutch)) +
  geom_raster(aes(fill=average)) +
  facet_wrap(~Foodweb) +
  xlab("Female preference (SDs)") +
  ylab("Clutch size (SDs)") +
...
P_matrix["Pref","Pref"] <- var(gall_selection.df$sc.log1p.Pref)
...
P_matrix["Diam","Pref"] <- with(gall_selection.df, cov(sc.Diam, sc.log1p.Pref))
P_matrix["Pref","Diam"] <- P_matrix["Diam","Pref"]

P_matrix["Clutch","Pref"] <- with(gall_selection.df, cov(sc.log.Clutch, sc.log1p.Pref))
...
densities <- gall_selection.df %>%
  distinct(Foodweb, Genotype, Plant_Position, sc.log1p.Pref, Density_per_100_shoots)
...
left_join(densities, survival) %>%
  ...
  ggplot(., aes(x=sc.log1p.Pref, y=ectos_sum/ptoids_sum)) +
...
left_join(densities, survival) %>%
  filter(Foodweb=="Complex") %>%
  ggplot(., aes(x=sc.log1p.Pref, y=egg_parasitoid_sum/ptoids_sum)) +
  geom_point(aes(size=ptoids_sum), shape=21) +
  geom_smooth(method = "glm", formula = y ~ x + I(x^2), method.args = list(family="binomial")) 

left_join(densities, survival) %>%
  ggplot(., aes(x=sc.log1p.Pref, y=gall_survival_sum/gall_survival_n, color=Foodweb)) +
  geom_point(aes(size=gall_survival_n), shape=21) +
  geom_smooth(method = "glm", formula = y ~ x + I(x^2), method.args = list(family="binomial"))

left_join(densities, survival) %>%
  ggplot(., aes(x=sc.log1p.Pref, y=gall_survival_n, color=Foodweb)) +
  geom_point(aes(size=gall_survival_n), shape=21) +
  geom_smooth(method = "lm", formula = y ~ x, aes(weight=gall_survival_n))

left_join(densities, survival) %>%
  filter(Foodweb=="Complex") %>%
  ggplot(., aes(x=sc.log1p.Pref)) +
  #geom_point(aes(size=ptoids_sum), shape=21) +
...
density_n_lm <- lm(gall_survival_n ~ sc.log1p.Pref*Foodweb, left_join(densities, survival))
visreg::visreg(density_n_lm, xvar="sc.log1p.Pref", by="Foodweb")
summary(density_n_lm)
wt_density_n_lm <- lm(gall_survival_n ~ sc.log1p.Pref*Foodweb, left_join(densities, survival), weights = gall_survival_n)
visreg::visreg(wt_density_n_lm, xvar="sc.log1p.Pref", by="Foodweb")
summary(wt_density_n_lm)

# good thing is that I get the same qualitative answer with or without weighting
test_glm <- glm(gall_survival_sum/gall_survival_n ~ Foodweb*(sc.log1p.Pref + I(sc.log1p.Pref^2)), left_join(densities, survival), family="quasibinomial")
visreg::visreg(test_glm, xvar="sc.log1p.Pref", by="Foodweb", scale="response")
summary(test_glm)

wt_test_glm <- glm(gall_survival_sum/gall_survival_n ~ Foodweb*(sc.log1p.Pref + I(sc.log1p.Pref^2)), left_join(densities, survival), family="quasibinomial", weights = gall_survival_n)
visreg::visreg(wt_test_glm, xvar="sc.log1p.Pref", by="Foodweb", scale="response")
summary(wt_test_glm)

# now with just egg parasitoids
egg.densities <- egg.vs.egg_df %>%
  distinct(Foodweb, Plant_Position, sc.log1p.Pref)
...
egg.test_glm <- glm(gall_survival_sum/gall_survival_n ~ Foodweb*(sc.log1p.Pref + I(sc.log1p.Pref^2)), left_join(egg.densities, egg.survival), family="quasibinomial")
visreg::visreg(egg.test_glm, xvar="sc.log1p.Pref", by="Foodweb", scale="response")
summary(egg.test_glm)

egg.wt_test_glm <- glm(gall_survival_sum/gall_survival_n ~ Foodweb*(sc.log1p.Pref + I(sc.log1p.Pref^2)), left_join(egg.densities, egg.survival), family="quasibinomial", weights = gall_survival_n)
visreg::visreg(egg.wt_test_glm, xvar="sc.log1p.Pref", by="Foodweb", scale="response")
...
ggplot(densities, aes(x=Genotype, y=sc.log1p.Pref, fill=Foodweb)) + geom_boxplot()
...
summary(lm(sc.log1p.Pref ~ Foodweb, densities))
...
ggplot(densities, aes(x=sc.log1p.Pref, fill=Foodweb)) + geom_density(alpha=0.5)
...
egg.egg_tidy_model_1 <- tidy(egg.egg_boot_model_1, conf.int=T) %>% 
  ...
  filter(term %in% c("I(sc.Diam^2)","I(sc.log.Clutch^2)","I(sc.log1p.Pref^2)",
                     "sc.Diam:sc.log.Clutch","sc.Diam:sc.log1p.Pref","sc.log.Clutch:sc.log1p.Pref",
                     "FoodwebComplex:I(sc.Diam^2)","FoodwebComplex:I(sc.log.Clutch^2)","FoodwebComplex:I(sc.log1p.Pref^2)",
                     "FoodwebComplex:sc.Diam:sc.log.Clutch","FoodwebComplex:sc.Diam:sc.log1p.Pref","FoodwebComplex:sc.log.Clutch:sc.log1p.Pref"))
```
...
egg.egg_plot_model_1$term_plot <- factor(egg.egg_plot_model_1$term,
                                levels=c("FoodwebComplex:I(sc.Diam^2)","FoodwebComplex:I(sc.log.Clutch^2)","FoodwebComplex:I(sc.log1p.Pref^2)","FoodwebComplex:sc.Diam:sc.log.Clutch","FoodwebComplex:sc.Diam:sc.log1p.Pref","FoodwebComplex:sc.log.Clutch:sc.log1p.Pref"),
                                ordered=TRUE)
...
visreg::visreg(egg.egg_model_1, xvar="sc.log1p.Pref", by="Foodweb", scale = "response")
```
...
tidy_egg.egg_model_3 <- tidy(boot_egg.egg_model_3, conf.int=T) 
```
...
egg.egg_plot_model_3$term_plot <- factor(egg.egg_plot_model_3$term,
                                levels=c("FoodwebComplex:sc.Diam","FoodwebComplex:sc.log.Clutch","FoodwebComplex:sc.log1p.Pref"),
                                ordered=TRUE)
...
tidy_egg.egg_model_4_adj$term_ord <- factor(tidy_egg.egg_model_4_adj$term, 
                                levels = c("(Intercept)","FoodwebSimple","sc.Diam","FoodwebSimple:sc.log.Clutch","FoodwebSimple:sc.log.Clutch","sc.log1p.Pref"),
                                ordered = TRUE)
...
tidy_egg.egg_common_coefs <- tidy_egg.egg_coefs %>%
  filter(term!="FoodwebEgg:I(sc.log1p.Pref^2)", term!="Foodwebegg:I(sc.log1p.Pref^2)",
         term!="FoodwebEgg:sc.log.Clutch",term!="FoodwebLarval:sc.log.Clutch")

# Tidy unique coefficients to each food-web
egg_quad_pref <- filter(tidy_egg.egg_coefs, term %in% "FoodwebEgg:I(sc.log1p.Pref^2)")
egg_quad_pref$term <- "I(sc.log1p.Pref^2)"
egg_quad_pref <- filter(tidy_egg.egg_coefs, term %in% "FoodwebLarval:I(sc.log1p.Pref^2)")
egg_quad_pref$term <- "I(sc.log1p.Pref^2)"
