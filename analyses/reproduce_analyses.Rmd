...
```{r Multivariate_QQ, fig.cap="\\label{fig:Multivariate_QQ}Multivariate quantile-quantile (Q-Q) plot to assess deviations from multivariate normality (black line)."}
...
```{r Plot egg vs. larval ptoid gradients}
...
```{r Egg vs. egg ptoid coefficients}
knitr::kable(tidy_eggegg_alphas)
```


```{r Plot egg vs. egg ptoid coefficients}
...
```{r Table of egg vs. egg ptoid gradients}
knitr::kable(tidy_eggegg_grads)
```


```{r Plot egg vs. egg ptoid gradients}
